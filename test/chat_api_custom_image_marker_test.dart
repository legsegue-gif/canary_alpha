...
void main() {
  group('ChatApiService structured media paths (no custom markers)', () {
    test('encodes local media paths as data URLs', () async {
      final body = await _sendAndCaptureRequestBody((baseUrl) async {
        final dir = await Directory.systemTemp.createTemp('canary_local_img_');
        addTearDown(() async {
          if (await dir.exists()) {
    ...
    test('assistant ImagePart has no image_url; media moves to following user', () async {
      final body = await _sendAndCaptureRequestBody((baseUrl) async {
        final dir = await Directory.systemTemp.createTemp('canary_chat_asst_img_');
        addTearDown(() async {
          if (await dir.exists()) {
            await dir.delete(recursive: true);
          }
        ...
        final body = await _sendAndCaptureRequestBody((baseUrl) async {
          return ChatApiService.sendMessageStream(
            ...
            messages: [
              {
                'role': 'assistant',
                // Pre-built multimodal List with no _canary_media_paths sidecar.
                'content': [
    ...
    test('userImagePaths attach images without marker strings in content', () async {
      final dir = await Directory.systemTemp.createTemp('canary_user_paths_');
      addTearDown(() async {
    ...
    test(
      ...
      () async {
        final dir =
            await Directory.systemTemp.createTemp('canary_tool_media_');
        addTearDown(() async {
          if (await dir.exists()) await dir.delete(recursive: true);
        });
        final file = File('${dir.path}/sample.png');
    ...
    test(
      ...
      () async {
        final dir =
            await Directory.systemTemp.createTemp('canary_tool_asst_media_');
        addTearDown(() async {
          if (await dir.exists()) await dir.delete(recursive: true);
        });
        final file = File('${dir.path}/assistant.png');
    ...
    test(
      ...
      () async {
        final dir =
            await Directory.systemTemp.createTemp('canary_tool_user_paths_');
        addTearDown(() async {
          if (await dir.exists()) await dir.delete(recursive: true);
        });
    ...
    test(
      ...
      () async {
        final dir =
            await Directory.systemTemp.createTemp('canary_list_user_asst_');
        addTearDown(() async {
          if (await dir.exists()) await dir.delete(recursive: true);
        });
        final file = File('${dir.path}/assistant.png');
  ...
  group('ChatApiService Responses API structured media paths', () {
    ...
    test('local video/mp4 is not encoded as input_image', () async {
      final body = await _sendAndCaptureResponsesBody((baseUrl) async {
        final dir = await Directory.systemTemp.createTemp('canary_resp_vid_');
        addTearDown(() async {
          if (await dir.exists()) {
            await dir.delete(recursive: true);
          }
    ...
    test('pure local video attachment keeps non-empty text content', () async {
      ...
      final body = await _sendAndCaptureResponsesBody((baseUrl) async {
        final dir = await Directory.systemTemp.createTemp('canary_resp_pure_vid_');
        addTearDown(() async {
          if (await dir.exists()) {
            await dir.delete(recursive: true);
          }
    ...
    test('encodes multimodalInternalMediaPathsKey as input_image', () async {
      final body = await _sendAndCaptureResponsesBody((baseUrl) async {
        final dir = await Directory.systemTemp.createTemp('canary_resp_img_');
        addTearDown(() async {
          if (await dir.exists()) {
            await dir.delete(recursive: true);
          }
    ...
    test('assistant ImagePart does not put input_image in assistant output', () async {
      final body = await _sendAndCaptureResponsesBody((baseUrl) async {
        final dir = await Directory.systemTemp.createTemp('canary_resp_asst_img_');
        addTearDown(() async {
          if (await dir.exists()) {
            await dir.delete(recursive: true);
          }
    ...
    test('multiple assistant images all attach to following user', () async {
      final body = await _sendAndCaptureResponsesBody((baseUrl) async {
        final dir = await Directory.systemTemp.createTemp('canary_resp_multi_asst_');
        addTearDown(() async {
          if (await dir.exists()) {
            await dir.delete(recursive: true);
          }
  ...
  group('Claude structured media paths (ticket 10)', () {
    test('encodes multimodalInternalMediaPathsKey as Anthropic image blocks', () async {
      final file = await _tempPng('canary_claude_media_');
      addTearDown(() async {
    ...
    test('skips missing local media paths without crashing', () async {
      final missing = '${Directory.systemTemp.path}/canary_missing_claude_${DateTime.now().microsecondsSinceEpoch}.png';
      final body = await _captureProviderBody(
        (baseUrl) {
          return ChatApiService.sendMessageStream(
            config: _claudeConfig(baseUrl),
    ...
    test('preserves media-paths list order in image blocks', () async {
      final dir = await Directory.systemTemp.createTemp('canary_claude_order_');
      addTearDown(() async {
        if (await dir.exists()) await dir.delete(recursive: true);
      });
    ...
    test('image/jpg supplemental mime is emitted as media_type image/jpeg', () async {
      final dir = await Directory.systemTemp.createTemp('canary_claude_jpg_');
      addTearDown(() async {
        if (await dir.exists()) await dir.delete(recursive: true);
      });
    ...
    test('video/mp4 supplemental ref does not become Claude image block', () async {
      final dir = await Directory.systemTemp.createTemp('canary_claude_video_');
      addTearDown(() async {
        if (await dir.exists()) await dir.delete(recursive: true);
      });
      final file = File('${dir.path}/clip.mp4');
  ...
  group('Gemini structured media paths (ticket 10)', () {
    test('encodes multimodalInternalMediaPathsKey as inline_data', () async {
      final file = await _tempPng('canary_gemini_media_');
      addTearDown(() async {
        final dir = file.parent;
        if (await dir.exists()) await dir.delete(recursive: true);
      });
    ...
    test('skips missing local media paths without crashing', () async {
      final missing = '${Directory.systemTemp.path}/canary_missing_gemini_${DateTime.now().microsecondsSinceEpoch}.png';
      final body = await _captureProviderBody(
        (baseUrl) {
          return ChatApiService.sendMessageStream(
            config: _geminiConfig('$baseUrl/v1beta'),
