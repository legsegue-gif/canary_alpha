...
const _canaryLink = '[daily_sign.py](canary://workspace/shenyu/daily_sign.py)';
...
void main() {
  ...
  Future<void> tapCanaryLinksAndExpectInApp(
    WidgetTester tester,
  ...
  testWidgets('canary link with underscore stays in-app in every context', (
    tester,
  ) async {
    final contexts = <String, String>{
      'paragraph': _canaryLink,
      'list': '- $_canaryLink',
      'bold': '**$_canaryLink**',
      'two on one line': '$_canaryLink $_canaryLink',
      'table': '| file |\n| --- |\n| $_canaryLink |',
      'chinese no space': '查看$_canaryLink',
    };

    for (final entry in contexts.entries) {
      launcher.launched.clear();
      final l10n = await pumpMarkdown(tester, entry.value);
      await tapCanaryLinksAndExpectInApp(tester, l10n);
    }
  });
}
