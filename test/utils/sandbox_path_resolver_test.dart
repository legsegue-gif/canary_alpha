...
void main() {
  ...
  group('SandboxPathResolver.fix canary-file', () {
    test('resolves canonical URI to absolute path without FS probe', () {
      const docs = '/tmp/canary_docs_does_not_need_to_exist';
      SandboxPathResolver.debugSetDirs(docsDir: docs);

      const uri = 'canary-file:///upload/missing_no_fs_probe.png';
    ...
    test('returns canary-file URI unchanged when docsDir is null', () {
      SandboxPathResolver.debugSetDirs(docsDir: null);
      const uri = 'canary-file:///images/photo.png';
      expect(SandboxPathResolver.fix(uri), uri);
    });

    test(
      'returns invalid canary-file URI unchanged (no strip/file fallback)',
      () {
        const docs = '/tmp/canary_docs';
        SandboxPathResolver.debugSetDirs(docsDir: docs);
        const uri = 'canary-file:not-a-valid-structure';
        expect(SandboxPathResolver.fix(uri), uri);
      },
    );
  });
  ...
  group('SandboxPathResolver.fix http/data pass-through', () {
    test('leaves http(s) and data URIs unchanged', () {
      SandboxPathResolver.debugSetDirs(docsDir: '/tmp/canary_docs');
  ...
  group('SandboxPathResolver.canonicalize', () {
    test('passes through remote / data / already-canonical URIs', () {
      ...
      expect(
        SandboxPathResolver.canonicalize('canary-file:///upload/a.png'),
        'canary-file:///upload/a.png',
      );
      // Cheap prefix: illegal structure still passes through.
      expect(
        SandboxPathResolver.canonicalize('canary-file:garbage'),
        'canary-file:garbage',
      );
    });
    ...
    test('does not guess external /images/ paths into canary-file', () {
      SandboxPathResolver.debugSetDirs(docsDir: '/tmp/canary_docs');
    ...
    test('decodes file:// percent-escapes before encoding', () {
      const docs = '/tmp/canary_docs';
      SandboxPathResolver.debugSetDirs(docsDir: docs);
      const fileUri = 'file:///tmp/canary_docs/upload/My%20Photo.png';
      expect(
        SandboxPathResolver.canonicalize(fileUri),
        'canary-file:///upload/My%20Photo.png',
      );
    });
    ...
    test('with docsDir set still encodes structured legacy UUID paths', () {
      SandboxPathResolver.debugSetDirs(docsDir: '/tmp/canary_docs_current');
      const legacy =
          '/var/mobile/Containers/Data/Application/A1B2C3D4-E5F6-7890-ABCD-EF1234567890/Documents/images/pic.png';
      expect(
        SandboxPathResolver.canonicalize(legacy),
    ...
    test('round-trips with encodeFromAbsolute helper', () {
      ...
      expect(uri, CanaryFileUri.encodeFromAbsolute(abs, root: docs));
  ...
  group('SandboxPathResolver.tryDecodeLocalFileUri', () {
    test('decodes local file URIs and rejects UNC/SMB hosts', () {
      final local = Uri.parse(
        'file:///tmp/canary_docs/upload/a.png',
      ).toFilePath();
      expect(
      ...
      expect(
        SandboxPathResolver.tryDecodeLocalFileUri(
          'file://localhost/tmp/canary_docs/upload/a.png',
        ),
        local,
      );
      expect(
  ...
  group('SandboxPathResolver.fix backslash handling', () {
    ...
    test('matches Windows drive paths with backslashes via Canary marker', () {
      ...
      SandboxPathResolver.debugSetDirs(docsDir: temp.path);
      // Use forward-slash Windows-like legacy form (lexical); ensure Canary
  ...
  group('SandboxPathResolver.tryRemapRestoredManagedAbsolute', () {
    ...
    test('maps old mac absolute root only when restored file exists', () {
      ...
      expect(
        SandboxPathResolver.tryRemapRestoredManagedAbsolute(
          '/Users/alice/Library/Application Support/app.canary.client/images/missing.png',
        ),
        isNull,
      );
