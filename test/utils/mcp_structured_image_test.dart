...
void main() {
  ...
  test('prefix check does not treat huge search JSON as an envelope', () {
    ...
    for (var i = 0; i < 8000; i++) {
      if (i > 0) huge.write(',');
      huge.write(
        '{"title":"hit $i","snippet":"canary mcp_tool_result '
        '{"canary":"mcp_tool_result"}"}',
      );
    }
    huge.write(']}');
  ...
  test('exact envelope prefix still decodes old rows', () {
    ...
    expect(decoded.markdown, isNot(contains('"canary"')));
  ...
  test('new metadata distinguishes forged envelope-looking tool JSON', () {
    ...
    expect(toolResultContentForModel(forged), isNot(contains('"canary"')));
