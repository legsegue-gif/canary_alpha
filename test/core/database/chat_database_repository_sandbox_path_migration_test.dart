  ...
  group('sandbox path migration version', () {
    ...
    setUp(() async {
      directory = await Directory.systemTemp.createTemp(
        'canary_sandbox_path_migration_',
      );
    ...
    test('canary-file URI stays canonical when targetRoot changes', () async {
        ...
        messages: [
          (
            message: ChatMessage(
              ...
              parts: [
                ImagePart(
                  uri: 'canary-file:///upload/canon.png',
                  unavailable: true,
                ),
              ],
            ),
      ...
      await repository.migrateSandboxPaths(
        targetVersion: 1,
        targetRoot: '/old-root',
        rewriteUri: (uri) => CanaryFileUri.isCanaryFileUri(uri) ? uri : uri,
      );

      final result = await repository.migrateSandboxPaths(
        targetVersion: 1,
        targetRoot: directory.path,
        rewriteUri: (uri) => CanaryFileUri.isCanaryFileUri(uri)
            ? uri
      ...
      final image = migrated.parts.whereType<ImagePart>().single;
      expect(image.uri, 'canary-file:///upload/canon.png');
      expect(image.unavailable, isFalse);
      expect(file.existsSync(), isTrue);
    });

    test('canary-file unavailable recomputed when file missing', () async {
      SandboxPathResolver.debugSetDirs(docsDir: directory.path);
      addTearDown(() {
        SandboxPathResolver.debugSetDirs(docsDir: null, supportDir: null);
      });
        ...
        messages: [
          (
            message: ChatMessage(
              ...
              parts: const [
                ImagePart(
                  uri: 'canary-file:///upload/does-not-exist.png',
                  unavailable: false,
                ),
              ],
            ),
      ...
      final image = migrated.parts.whereType<ImagePart>().single;
      expect(image.uri, 'canary-file:///upload/does-not-exist.png');
      expect(image.unavailable, isTrue);
    });

    test(
      'canonicalize rewrite persists canary-file and skips UNC I/O',
      () async {
        ...
        expect(parts[0].uri, 'canary-file:///upload/a.png');
