...
void main() {
  group('ChatDatabaseRepository merge snapshot', () {
    ...
    setUp(() async {
      directory = await Directory.systemTemp.createTemp('canary_merge_test_');
    ...
    test('附件 ordinal 顺序不同产生不同指纹', () async {
      await putConversationWithAttachments(
        ...
        content: 'same body',
        parts: const [
          ImagePart(uri: 'canary-file:///upload/a.png', mime: 'image/png'),
          ImagePart(uri: 'canary-file:///upload/b.png', mime: 'image/png'),
        ],
      );
      await putConversationWithAttachments(
        source,
