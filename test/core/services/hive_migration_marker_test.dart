...
void main() {
  ...
  setUp(() async {
    directory = await Directory.systemTemp.createTemp('canary_hive_marker_');
  });
  ...
  File writeDatabase({
    ...
    final file = fileNamed('canary.db');
  ...
  test('reports not migrated for a database that was never initialised', () {
    ...
    final file = fileNamed('canary.db')..writeAsBytesSync(const []);
  ...
  test('refuses a truncated database header', () {
    final file = fileNamed('canary.db')..writeAsStringSync('SQLite fo');
    expect(
      () => HiveMigrationMarker.requireMigrationComplete(file),
      throwsCorrupt(),
    );
