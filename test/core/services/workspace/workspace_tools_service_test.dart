...
import 'package:Canary/core/models/environment_variable.dart';
...
import 'package:Canary/core/services/workspace/tool_run_registry.dart';
...
void main() {
  ...
  setUp(() async {
    tmp = await Directory.systemTemp.createTemp('canary_ws_tools_');
  ...
  test(
    ...
    () async {
      ...
      expect(
        ...
        Platform.isWindows
            ? 'canary://workspace/report%20%E6%8A%A5%E5%91%8A.txt'
            : 'canary://workspace/report%3A%E6%8A%A5%E5%91%8A.txt',
      );
      expect(
        CanaryLink.tryParse(write.files.single.link!)?.relativePath,
        fileName,
      );
  ...
  test(
    ...
    () async {
      ...
      for (final path in ['/mounts/Data/a.txt', '/chat/note.txt']) {
        ...
        expect(
          ...
          path.startsWith('/mounts')
              ? 'canary://mounts/mount-1/a.txt'
              : 'canary://session/note.txt',
        );
      }
      ...
      expect(listing.files.first.link, 'canary://mounts/mount-1');
      expect(listing.files.last.link, 'canary://mounts/mount-1/a.txt');
      final glob = metaOf(
        await tools.handle(context, 'glob', {
          'path': '/mounts/Data',
          'pattern': '*.txt',
  ...
  test('runtime failures still report files already written', () async {
    for (final throwError in [true, false]) {
      ...
      expect(result.files.single.link, 'canary://workspace/partial.txt');
      expect(result.files.single.isProduced, isTrue);
    ...
    test('fromJson(toJson) round trip', () {
      final original = WorkspaceToolMetadata(
        ...
        files: const [
          WorkspaceToolFile(
            path: '/workspace/a.txt',
            link: 'canary://workspace/a.txt',
            role: WorkspaceFileRole.modified,
          ),
          WorkspaceToolFile(
            path: '/chat/outputs/t.txt',
            link: 'canary://chat/outputs/t.txt',
