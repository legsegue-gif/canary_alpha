...
void main() {
  ...
  setUp(() async {
    ...
    tmp = await Directory.systemTemp.createTemp('canary_desktop_runtime_');
  });
    ...
    test('passes request env through to the process', () async {
      final events = await _collect(
        ...
        _req(
          ...
          cwd: tmp.path,
          env: const <String, String>{'CANARY_TEST': 'canary-runtime-ok'},
        ),
      );
      expect(utf8.decode(_stdout(events)), contains('canary-runtime-ok'));
      expect(_singleExit(events).exitCode, 0);
    }, skip: !isUnix);
