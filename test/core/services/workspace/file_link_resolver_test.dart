...
import 'package:Canary/core/models/workspace_directory_access.dart';
...
import 'package:Canary/core/models/workspace_binding.dart';
import 'package:Canary/core/providers/workspace_provider.dart';
import 'package:Canary/core/services/workspace/file_link_resolver.dart';
import 'package:Canary/core/services/workspace/workspace_paths.dart';
import 'package:Canary/core/services/workspace/workspace_runtime.dart';
...
void main() {
  ...
  group('CanaryLink.tryParse', () {
    test('parses workspace, chat, skill, and terminal kinds', () {
      final workspace = CanaryLink.tryParse(
        'canary://workspace/docs/readme.md',
      );
      expect(workspace?.kind, CanaryLinkKind.workspaceFile);
      expect(workspace?.relativePath, 'docs/readme.md');

      final attachment = CanaryLink.tryParse(
        'canary://chat/attachments/photo.png',
      );
      expect(attachment?.kind, CanaryLinkKind.chatAttachment);
      expect(attachment?.relativePath, 'photo.png');

      final output = CanaryLink.tryParse('canary://chat/outputs/result.json');
      expect(output?.kind, CanaryLinkKind.chatOutput);
      expect(output?.relativePath, 'result.json');

      final skill = CanaryLink.tryParse('canary://skills/weather/SKILL.md');
      expect(skill?.kind, CanaryLinkKind.skillFile);
      expect(skill?.relativePath, 'weather/SKILL.md');

      final terminal = CanaryLink.tryParse('canary://terminal?cmd=ls%20-la');
      expect(terminal?.kind, CanaryLinkKind.terminal);
    ...
    test('directory references retain complete guest paths for copying', () {
      const cases = {
        'canary://workspace/reports': '/workspace/reports',
        'canary://chat/attachments/incoming': '/chat/attachments/incoming',
        'canary://chat/outputs/reports': '/chat/outputs/reports',
        'canary://session/notes': '/chat/notes',
        'canary://skills/skill-id': '/skills/skill-id',
        'canary://tmp/build': '/tmp/build',
        'canary://mounts/mount-id/reports': '/mounts/Renamed/reports',
      };
    ...
    test('accepts underscores in workspace paths', () {
      final link = CanaryLink.tryParse(
        'canary://workspace/shenyu/daily_sign.py',
      );
      expect(link?.kind, CanaryLinkKind.workspaceFile);
      expect(link?.relativePath, 'shenyu/daily_sign.py');
    });

    test('percent-decodes path segments', () {
      final link = CanaryLink.tryParse('canary://workspace/hello%20world.txt');
      expect(link?.kind, CanaryLinkKind.workspaceFile);
      expect(link?.relativePath, 'hello world.txt');
    });

    test('accepts raw UTF-8 and encoded Chinese workspace names', () {
      final raw = CanaryLink.tryParse('canary://workspace/员工表.csv');
      expect(raw?.kind, CanaryLinkKind.workspaceFile);
      expect(raw?.relativePath, '员工表.csv');

      final encoded = CanaryLink.tryParse(
        'canary://workspace/${Uri.encodeComponent('员工表.csv')}',
      );
      expect(encoded?.kind, CanaryLinkKind.workspaceFile);
      expect(encoded?.relativePath, '员工表.csv');
    });

    test('decodes encoded directory segments', () {
      final link = CanaryLink.tryParse(
        'canary://workspace/sub%20dir/a%20b.txt',
      );
      expect(link?.kind, CanaryLinkKind.workspaceFile);
      expect(link?.relativePath, 'sub dir/a b.txt');
    });

    test('parses canary://chat/<id>/Chinese filename as chat output', () {
      const conversationId = 'conv-中文';
      final link = CanaryLink.tryParse('canary://chat/$conversationId/输出.png');
      expect(link?.kind, CanaryLinkKind.chatOutput);
      expect(link?.conversationId, conversationId);
      expect(link?.relativePath, '输出.png');
    });

    test('rejects .. segments and encoded traversal', () {
      expect(CanaryLink.tryParse('canary://workspace/../secret'), isNull);
      expect(
        CanaryLink.tryParse('canary://workspace/foo/../../etc/passwd'),
        isNull,
      );
      expect(CanaryLink.tryParse('canary://workspace/%2e%2e/secret'), isNull);
      expect(CanaryLink.tryParse('canary://workspace/%2e%2e%2fsecret'), isNull);
      expect(CanaryLink.tryParse('canary://chat/attachments/../x'), isNull);
      expect(CanaryLink.tryParse('canary://skills/foo/../bar'), isNull);
    });

    test('rejects absolute-host and drive paths', () {
      expect(CanaryLink.tryParse('canary://workspace//etc/passwd'), isNull);
      expect(
        CanaryLink.tryParse('canary://workspace/C:/Windows/win.ini'),
        isNull,
      );
      expect(CanaryLink.tryParse('canary:///workspace/foo'), isNull);
    });

    test('rejects unknown hosts and incomplete chat/skill paths', () {
      expect(CanaryLink.tryParse('canary://other/foo'), isNull);
      expect(
        CanaryLink.tryParse('canary://chat/attachments')?.relativePath,
        isEmpty,
      );
      expect(
        CanaryLink.tryParse('canary://skills/only-id')?.relativePath,
        'only-id',
      );
      expect(CanaryLink.tryParse('https://example.com'), isNull);
      expect(CanaryLink.tryParse(''), isNull);
    });
  });
  ...
  group('FileLinkResolver.resolveToHostFile', () {
    ...
    setUp(() async {
      tempDir = await Directory.systemTemp.createTemp('canary_file_link_');
    ...
    test('resolves a workspace file under the host root', () async {
      ...
      final link = CanaryLink.tryParse('canary://workspace/foo.txt');
    ...
    test('returns null for a missing workspace file', () async {
      final link = CanaryLink.tryParse('canary://workspace/missing.txt')!;
      final resolved = await resolver.resolveToHostFile(
        link,
        conversationId: 'conv-1',
        binding: binding(),
    ...
    test(
      ...
      () async {
        ...
        final attachResolved = await resolver.resolveToHostFile(
          CanaryLink.tryParse('canary://chat/attachments/a.txt')!,
          conversationId: conversationId,
          binding: const WorkspaceBinding(),
        );
        final outputResolved = await resolver.resolveToHostFile(
          CanaryLink.tryParse('canary://chat/outputs/b.txt')!,
          conversationId: conversationId,
          binding: const WorkspaceBinding(),
        );
    ...
    test('resolves skill files and rejects skill escapes', () async {
      ...
      final resolved = await resolver.resolveToHostFile(
        CanaryLink.tryParse('canary://skills/weather/SKILL.md')!,
        conversationId: 'conv-1',
        binding: const WorkspaceBinding(),
      );
    ...
    test(
      ...
      () async {
        ...
        final parsed = CanaryLink.tryParse(href!);
        expect(parsed?.kind, CanaryLinkKind.workspaceFile);
    ...
    test('resolves encoded chat output with Chinese name', () async {
      ...
      final parsed = CanaryLink.tryParse(
        'canary://chat/$conversationId/${Uri.encodeComponent('输出.png')}',
      );
    ...
    test(
      ...
      () async {
        final rootLink = CanaryLink.tryParse('canary://workspace/')!;
        ...
        await expectLater(
          resolver.resolveToHostEntry(
            CanaryLink.tryParse('canary://workspace/gone.txt')!,
            conversationId: 'conv-1',
            binding: binding(),
          ),
          throwsA(
        ...
        for (final path in [
          ...
          expect(
            await resolver.resolveToHostEntry(
              CanaryLink.tryParse('canary://workspace/$path')!,
              conversationId: 'conv-1',
              binding: binding(),
            ),
            isNull,
    ...
    test(
      ...
      () async {
        ...
        try {
          ...
          final parsed = CanaryLink.tryParse(link)!;
          ...
          final rootLink = CanaryLink.tryParse(
            'canary://mounts/${parsed.mountId}',
          )!;
          expect(
    ...
    test('terminal links do not resolve to a file', () async {
      final resolved = await resolver.resolveToHostFile(
        CanaryLink.tryParse('canary://terminal?cmd=pwd')!,
        conversationId: 'conv-1',
        binding: binding(),
      );
      expect(resolved, isNull);
