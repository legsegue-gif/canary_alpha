...
void main() {
  test(
    ...
    () async {
      final temp = await Directory.systemTemp.createTemp('canary_ios_env_');
      ...
      final compile = await Process.run('xcrun', [
        ...
        'ios/Runner/Workspace/CanaryISHEnvironment.m',
  ...
  for (final shell in ['/bin/bash', '/bin/sh']) {
    test(
      ...
      () async {
        ...
        await File(
          'ios/sandbox/overlay/etc/profile.d/canary.sh',
        ).copy('${profiles.path}/canary.sh');
