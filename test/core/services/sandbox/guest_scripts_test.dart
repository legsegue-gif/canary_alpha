...
void main() {
  test('applyAptMirror golden', () {
    expect(
      ...
      '  mv /etc/apt/sources.list /etc/apt/sources.list.canary-bak\n'
      'fi\n'
      'set -e\n'
  ...
  for (final distro in ['ubuntu', 'debian']) {
    test(
      ...
      () async {
        ...
        expect(
          await File('${legacy.path}.canary-bak').readAsString(),
          'old system source',
        );
  ...
  test(
    ...
    () async {
      final dir = await Directory.systemTemp.createTemp('canary_source_');
      addTearDown(() => dir.delete(recursive: true));
