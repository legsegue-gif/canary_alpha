...
void main() {
  ...
  group('DataSync.prepareBackupFileFromDatabase', () {
    ...
    setUp(() async {
      root = await Directory.systemTemp.createTemp('canary_foreign_db_');
    ...
    test('把一份独立数据库打成标准备份归档', () async {
      final source = await writeDatabase('canary.db.displaced-0001', 'aside');
    ...
    test('归档自带数据库，不引用原文件', () async {
      final source = await writeDatabase('canary.db.displaced-0002', 'aside');
      final live = await writeDatabase('live.sqlite', 'current');
      final liveDatabase = AppDatabase.open(file: live);
      File? archive;
      try {
        ...
        final entries = ZipDecoder()
            ...
            .map((file) => file.name)
            .toSet();
        expect(entries, contains('database/canary.db'));
    ...
    test('不修改源副本', () async {
      final source = await writeDatabase('canary.db.displaced-0003', 'aside');
