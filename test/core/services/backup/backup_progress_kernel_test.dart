...
void main() {
  ...
  group('runBackupIsolate', () {
    ...
    test(
      ...
      () async {
        ...
        final heartbeat = File(
          '${Directory.systemTemp.path}/canary_backup_timeout_${identityHashCode(token)}.hb',
        );
        addTearDown(() async {
    ...
    test(
      ...
      () async {
        final closedMarker = File(
          '${Directory.systemTemp.path}/canary_sqlite_close_after_timeout_${DateTime.now().microsecondsSinceEpoch}.marker',
        );
    ...
    test(
      ...
      () async {
        final closedMarker = File(
          '${Directory.systemTemp.path}/canary_sqlite_closed_${DateTime.now().microsecondsSinceEpoch}.marker',
        );
        if (closedMarker.existsSync()) closedMarker.deleteSync();
        addTearDown(() {
          if (closedMarker.existsSync()) closedMarker.deleteSync();
