...
import 'package:Canary/core/services/backup/chatbox_importer.dart';
...
void main() {
  ...
  group('ChatboxImporter SQLite business patch', () {
    ...
    setUp(() async {
      root = await Directory.systemTemp.createTemp('canary_chatbox_db_');
    ...
    test(
      ...
      () async {
        ...
        final reloaded = ChatService(
          existingRepository: ChatDatabaseRepository(
            database,
            databaseFile: File('${root.path}/canary.db'),
          ),
        );
        await reloaded.init();
        try {
