...
Future<Directory> _singleRestoreRunDirectory(Directory appDataDirectory) async {
  final workspace = Directory(
    '${appDataDirectory.path}${Platform.pathSeparator}.canary_restore',
  );
...
void main() {
  group('DataSync backup file', () {
    ...
    setUp(() async {
      root = await Directory.systemTemp.createTemp('canary_data_sync_test_');
    ...
    test(
      ...
      () async {
        ...
        final staleWorkDir = Directory(
          '${tmpDir.path}/canary_backup_2000-01-01T00-00-00.000000',
        );
        await staleWorkDir.create(recursive: true);
        await File('${staleWorkDir.path}/orphan.zip').writeAsString('old');
        final staleZip = File(
          '${tmpDir.path}/canary_backup_2000-01-01T00-00-01.000000.zip',
        );
    ...
    test('cancelling packing deletes the work directory', () async {
      ...
      if (await tmp.exists()) {
        ...
        expect(
          leftovers.where(
            (name) =>
                name.startsWith('canary_backup_') ||
    ...
    test('writes a consistent SQLite snapshot instead of chats.json', () async {
      ...
      try {
        archive = ZipDecoder().decodeStream(input);
        final manifestEntry = archive.findFile('manifest.json');
        final databaseEntry = archive.findFile('database/canary.db');
        ...
        final manifest =
            jsonDecode(utf8.decode(manifestEntry!.readBytes()!))
                as Map<String, dynamic>;
        expect(manifest['format'], 'canary-backup');
        ...
        expect(
          ((manifest['entries'] as Map)['database/canary.db'] as Map)['sha256'],
          archivedHash,
        );
      } finally {
        archive?.clearSync();
    ...
    test('restores a prepared SQLite snapshot on the next startup', () async {
      ...
      encoder.addFileSync(manifestFile, 'manifest.json');
      encoder.addFileSync(settingsFile, 'settings.json');
      encoder.addFileSync(sourceFile, 'database/canary.db');
      encoder.closeSync();
    ...
    test('retains a prepared SQLite candidate under app data', () async {
      ...
      final workspace = Directory('${root.path}/.canary_restore');
      ...
      final candidateDatabase = File(
        '${runDirectory.path}/candidate/database/canary.db',
      );
    ...
    test(
      ...
      () async {
        ...
        expect(
          await Directory('${root.path}/.canary_restore').exists(),
          isFalse,
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        await Link('${root.path}/.canary_restore').create(outside.path);
        final zipFile = await _createSqliteBackupFixture(
          root: root,
    ...
    test('retains every selected SQLite and asset component', () async {
      ...
      expect(
        ...
        containsAll(['database/canary.db', 'upload/fixture.txt']),
      );
      expect(
      ...
      expect(
        await File(p.join(candidate.path, 'database', 'canary.db')).exists(),
        isTrue,
      );
      expect(
        await File(
  ...
  group('canary-file portable attachments', () {
    test('resolve after root change without rewriting persisted URIs', () async {
      ...
      final dbFile = File('${rootA.path}/canary.db');
      ...
      const uri = 'canary-file:///upload/pic.png';
      await repository.putMigrationBatch(
        conversations: [
          Conversation(
            id: 'c-portable',
      ...
      final dbB = File('${rootB.path}/canary.db');
      ...
      final result = await restoredRepo.migrateSandboxPaths(
        ...
        rewriteUri: (value) => CanaryFileUri.isCanaryFileUri(value)
            ? value
