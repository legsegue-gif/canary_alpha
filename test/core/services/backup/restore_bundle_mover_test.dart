...
import 'package:Canary/core/services/backup/restore_bundle_mover.dart';
  ...
  group('RestoreBundleMover', () {
    ...
    setUp(() async {
      appData = await Directory.systemTemp.createTemp(
        'canary_restore_bundle_mover_test_',
      );
    ...
    test('syncs every old asset before the first bundle rename', () async {
      ...
      final databaseRename = durability.events.indexWhere(
        (event) => event.endsWith('/previous.pending/database/canary.db'),
      );
...
Future<_CutoverFixture> _prepareCutoverFixture({
  ...
  final candidateDatabase = File(
    p.join(candidateDirectory.path, 'database', 'canary.db'),
  );
  ...
  await manifest.writeAsString(
    jsonEncode({
      ...
      'entries': {
        'database/canary.db': databaseDescriptor,
