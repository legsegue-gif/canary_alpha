...
import 'package:Canary/core/services/backup/restore_live_database.dart';
  ...
  group('RestoreLiveDatabase', () {
    ...
    setUp(() async {
      root = await Directory.systemTemp.createTemp(
        'canary_restore_live_database_test_',
      );
    });
    ...
    test('preserves a completely missing database family', () async {
      final database = File(p.join(root.path, 'canary.db'));
    ...
    test('checkpoints a crash-style WAL family into one main file', () async {
      ...
      final target = File(p.join(root.path, 'canary.db'));
    ...
    test('normalizes WAL mode even after its sidecars disappeared', () async {
      final databaseFile = File(p.join(root.path, 'canary.db'));
    ...
    test('rejects linked main files without touching the target', () async {
      ...
      final link = Link(p.join(root.path, 'canary.db'));
