...
void main() {
  group('DatabaseChangeFingerprint', () {
    ...
    setUp(() async {
      root = await Directory.systemTemp.createTemp('canary_fingerprint_');
      database = File(p.join(root.path, 'canary.db'));
  ...
  group('LocalSnapshotPaths', () {
    ...
    test('不认得的名字返回 null，不会被当成副本', () {
      expect(LocalSnapshotPaths.createdAtFromFileName('canary.db'), isNull);
      expect(
        LocalSnapshotPaths.createdAtFromFileName(
