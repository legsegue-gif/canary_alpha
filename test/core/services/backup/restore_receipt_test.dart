...
Future<String> _writeCandidateManifest(RestoreReceiptStore store) async {
  ...
  final database = File(p.join(candidate.path, 'database', 'canary.db'));
...
void main() {
  ...
  group('RestoreReceiptStore', () {
    ...
    setUp(() async {
      root = await Directory.systemTemp.createTemp('canary_receipt_test_');
    ...
    test(
      ...
      () async {
        await File(
          p.join(store.runDirectory.path, 'candidate', 'database', 'canary.db'),
        ).delete();
