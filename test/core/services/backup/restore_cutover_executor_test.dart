...
import 'package:Canary/core/services/backup/restore_bundle_preparation.dart';
import 'package:Canary/core/services/backup/restore_cutover_executor.dart';
...
void main() {
  ...
  group('RestoreCutoverExecutor', () {
    ...
    test('commits database and assets in the first startup pass', () async {
      final liveDatabase = File(p.join(appData.path, 'canary.db'));
      ...
      expect(
        await _conversationIds(
          File(p.join(previous.path, 'database', 'canary.db')),
        ),
        ['old'],
      );
      expect(
    ...
    test('commits when the previous database was absent', () async {
      ...
      expect(terminal.state, RestoreReceiptState.committed);
      expect(await _conversationIds(File(p.join(appData.path, 'canary.db'))), [
        'new',
      ]);
      final manifest =
          (jsonDecode(
    ...
    test('rolls back an interrupted candidate database install', () async {
      ...
      expect(
        await _conversationIds(
          File(
            p.join(prepared.candidateDirectory.path, 'database', 'canary.db'),
          ),
        ),
        ['new'],
      );
...
Future<PreparedRestoreBundle> _prepareBundle({
  ...
  final entries = <String, dynamic>{
    ...
    'database/canary.db': await _descriptor(database),
  };
  if (includeFiles) {
...
final class _ThrowAfterCandidateDatabaseRename implements RestoreDurability {
  ...
  Future<void> renameAndSync({
    ...
    if (!_didThrow &&
        p.basename(source.path) == 'canary.db' &&
        p.basename(source.parent.path) == 'database' &&
        p.equals(targetPath, p.join(appDataDirectory.path, 'canary.db')) &&
