...
import 'package:Canary/core/models/token_usage.dart';
...
import 'package:Canary/core/services/api/generation/text_generation_result.dart';
...
void main() {
  ...
  test('a generated file becomes an image part only when it is one', () {
    ...
    handler.handle(
      const GeneratedFile(
        uri: 'canary-file:///upload/chart.png',
        name: 'chart.png',
        mime: 'image/png',
      ),
    );
    handler.handle(
      const GeneratedFile(
        uri: 'canary-file:///upload/data.csv',
        name: 'data.csv',
        mime: 'text/csv',
      ),
    );
    ...
    expect(image.uri, 'canary-file:///upload/chart.png');
    expect(image.mime, 'image/png');
    final file = handler.parts[1] as FilePart;
    expect(file.uri, 'canary-file:///upload/data.csv');
  ...
  test('places a tool call between reasoning and later text', () {
    ...
    handler.handle(
      const ToolCallDelta(id: 'call_1', inputDelta: '{"q":"canary"}'),
    );
    ...
    expect(tool['arguments'], <String, dynamic>{'q': 'canary'});
  ...
  test('fills a local ToolCallResult without marking the part server-side', () {
    ...
    expect(payload['arguments'], <String, dynamic>{'q': 'canary'});
    ...
    handler.handleResult(
      const TextGenerationResult(
        parts: [
          ...
          ImagePart(uri: 'canary-file:///images/a.png', mime: 'image/png'),
        ],
        finishReason: 'stop',
      ),
    );
