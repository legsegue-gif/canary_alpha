...
import 'package:Canary/core/services/api/providers/claude/claude_files.dart';
...
void main() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp('canary_claude_files');
  ...
  group('downloadClaudeGeneratedFile', () {
    test('stores the file and names it as the container did', () async {
      ...
      expect(CanaryFileUri.isCanaryFileUri(file.uri), isTrue);
...
void uploadTests() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp('canary_claude_upload');
  });
