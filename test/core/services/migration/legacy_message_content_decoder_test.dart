...
import 'package:Canary/core/services/migration/legacy_message_content_decoder.dart';
...
void main() {
  ...
  test('managed Documents absolute path becomes canary-file URI', () async {
    ...
    expect(part.uri, 'canary-file:///upload/shot.png');
    ...
    expect(
      ...
      'canary-file:///upload/shot.png',
    );
  });

  test(
    'missing managed file still yields unavailable canary-file part',
    () async {
      final docs = Directory('${tempDir.path}/Documents')..createSync();
      SandboxPathResolver.debugSetDirs(docsDir: docs.path);
      addTearDown(() => SandboxPathResolver.debugSetDirs());
      ...
      expect(result.missingFiles, 1);
      final part = result.parts.single as ImagePart;
      expect(part.uri, 'canary-file:///upload/gone.png');
      expect(part.unavailable, isTrue);
    },
  );

  test('ordinary user Documents path is not claimed as canary-file', () async {
