...
import 'package:Canary/core/services/asr/cloud_asr_service.dart';
    ...
    test('uses the official Qwen ASR realtime lifecycle', () async {
      ...
      final session =
          await CloudAsrService(
            ...
            DashScopeAsrOptions(
              apiKey: 'dash-secret',
              websocketUrl: 'wss://dash.example.test/realtime?workspace=canary',
              language: 'zh',
            ),
          );
      ...
      expect(connectedUri.queryParameters, containsPair('workspace', 'canary'));
      ...
      expect(socket.sentJson.last['event_id'], 'canary_asr_1');
      ...
      expect(socket.sentJson.last['event_id'], 'canary_asr_3');
      socket.serverJson({
        'type': 'conversation.item.input_audio_transcription.completed',
        'item_id': 'dash-item',
        'transcript': '阿里云',
