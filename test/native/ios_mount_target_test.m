...
int main(void) {
    @autoreleasepool {
        ...
        check(CanaryISHValidateMountTarget(data, @"/mounts/Notes") == 0, "missing target rejected");
        check([fm createDirectoryAtPath:target withIntermediateDirectories:YES attributes:nil error:nil], "mkdir failed");
        check(CanaryISHValidateMountTarget(data, @"/mounts/Notes") == 0, "empty placeholder rejected");
        ...
        check(CanaryISHValidateMountTarget(data, @"/mounts/Notes") == CanaryISHMountTargetOccupied, "nonempty guest directory accepted");
        ...
        check(CanaryISHValidateMountTarget(data, @"/mounts/Notes") == CanaryISHMountTargetOccupied, "hidden file ignored");
        ...
        check(CanaryISHValidateMountTarget(data, @"/mounts/Notes") == CanaryISHMountTargetOccupied, "nested directory ignored");
        ...
        check(CanaryISHValidateMountTarget(data, @"/mounts/Notes") == CanaryISHMountTargetOccupied, "plain guest file accepted");
        ...
        check(CanaryISHValidateMountTarget(data, @"/mounts/Notes") == 0, "existing bind followed into nonempty source");
        ...
        check(CanaryISHValidateMountTarget(data, @"/mounts/Notes") == 0, "stale bind symlink rejected");
        ...
        check(CanaryISHValidateMountTarget(data, @"/mounts/file/child") < 0, "inspection failure accepted");
