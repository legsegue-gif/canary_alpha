...
int main(void) {
    @autoreleasepool {
        ...
        for (NSString *name in @[@"a", @"b"]) {
            ...
            NSData *context = CanaryISHCreateFilesystem(binds);
        ...
        for (int pass = 0; pass < 3; pass++) {
            for (int index = 0; index < 2; index++) {
                ...
                for (NSString *guest in @[@"/workspace", @"/chat", @"/skills", @"/tmp"]) {
                    ...
                    check(CanaryISHTranslatePath(path.UTF8String, inheritedContext, host, sizeof(host)), "forward translation");
                    ...
                    check(CanaryISHReversePath(host, inheritedContext, reverse, sizeof(reverse)), "reverse translation");
                    check(strcmp(reverse, path.UTF8String) == 0, "cwd round trip");
                    check(!CanaryISHTranslatePath("/workspace-other/file", inheritedContext, host, sizeof(host)), "prefix boundary");
                }
            }
        }
        char out[PATH_MAX];
        check(!CanaryISHTranslatePath("/workspace/file", 0, out, sizeof(out)), "unbound context");
        check(CanaryISHCreateFilesystem(@[@{@"guest":@"/workspace", @"host":@"/missing-canary-test-root"}]) == nil, "missing root accepted");
