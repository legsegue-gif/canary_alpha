...
import 'package:Canary/core/services/logging/context_log_models.dart';
import 'package:Canary/core/services/logging/context_logger.dart';
import 'package:Canary/core/services/search/search_tool_service.dart';
...
bool _hasSegmentsKey(List<Map<String, dynamic>> messages) {
  return messages.any(
    (message) => message.containsKey(canaryContextSegmentsKey),
  );
}
...
void main() {
  ...
  test(
    'logger disabled leaves _canary_ctx_segments off buildApiMessages and injects',
    () {
  ...
  test('stripInternalRevisionIds removes the segments key', () async {
    ...
    expect(apiMessages.first.containsKey(canaryContextSegmentsKey), isTrue);
    expect(
