...
import 'package:Canary/core/services/workspace/workspace_tool_metadata.dart';
import 'package:Canary/core/services/workspace/file_link_resolver.dart';
import 'package:Canary/features/chat/widgets/produced_files_row.dart';
import 'package:Canary/features/chat/widgets/workspace_tool_ui.dart';
        ...
        for (var i = 0; i < links.length; i++)
          WorkspaceToolFile(
            path: files != null && i < files.length
                ? files[i]
                : CanaryLink.tryParse(links[i])?.relativePath ?? links[i],
            link: links[i],
...
void main() {
  test('dedupes produced files by first appearance', () {
    final entries = collectProducedFileEntries([
      _write(links: ['canary://workspace/a.txt']),
      _write(links: ['canary://workspace/b.txt']),
      _write(links: ['canary://workspace/a.txt']),
    ]);
    expect(entries.map((e) => e.dedupeKey).toList(), [
      'canary://workspace/a.txt',
      'canary://workspace/b.txt',
    ]);
  });
  ...
  test(
    ...
    () {
      ...
      const result = WorkspaceToolFile(
        ...
        link: 'canary://workspace/result.csv',
  ...
  test('marks image links', () {
    final entries = collectProducedFileEntries([
      _write(links: ['canary://workspace/plot.PNG']),
      _write(links: ['canary://workspace/note.txt']),
    ]);
  ...
  test(
    ...
    () {
      const cases = {
        'canary://workspace/%E6%96%B0%E6%96%87%E4%BB%B6.txt': '新文件.txt',
        'canary://chat/outputs/报告%20終稿.txt': '报告 終稿.txt',
        'canary://workspace/資料/한글.txt': '資料/한글.txt',
        'canary://workspace/literal%2520%25.txt': 'literal%20%.txt',
        'canary://workspace/invalid%ZZ.txt':
            'canary://workspace/invalid%ZZ.txt',
      };
  ...
  test('keeps explicit file paths literal', () {
    ...
    final entry = collectProducedFileEntries([
      _write(links: ['canary://workspace/原始%2520文件.txt'], files: [path]),
    ]).single;
  ...
  testWidgets('shows a decoded CJK name in the produced file chip', (
    ...
    const link = 'canary://workspace/%E6%96%B0%E6%96%87%E4%BB%B6.txt';
  ...
  testWidgets('limits visible chips and shows +N', (tester) async {
    final parts = <WorkspaceToolPart>[
      for (var i = 0; i < 15; i++)
        _write(links: ['canary://workspace/file_$i.txt']),
    ];
