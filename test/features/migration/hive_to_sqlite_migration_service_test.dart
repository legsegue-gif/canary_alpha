...
import 'package:Canary/core/services/hive_migration_marker.dart';
...
void main() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp(
      'canary_hive_sqlite_migration_test_',
    );
  ...
  test('backs up Hive files and migrates chat data into SQLite', () async {
    ...
    expect(entryNames, isNot(contains('database/canary.db')));
    ...
    final repo = ChatDatabaseRepository.open(
      file: File('${tempDir.path}/canary.db'),
    );
  ...
  test(
    ...
    () async {
      ...
      expect(File('${tempDir.path}/canary.db.migrating').existsSync(), isFalse);
  ...
  test(
    ...
    () async {
      ...
      expect(File('${tempDir.path}/conversations.hive').existsSync(), isTrue);
      expect(File('${tempDir.path}/messages.hive').existsSync(), isTrue);
      expect(File('${tempDir.path}/canary.db').existsSync(), isFalse);
      expect(File('${tempDir.path}/canary.db.migrating').existsSync(), isFalse);
  ...
  test(
    ...
    () async {
      ...
      await Directory('${tempDir.path}/canary.db').create();
      final staleTemp = File('${tempDir.path}/canary.db.migrating');
      final staleWal = File('${tempDir.path}/canary.db.migrating-wal');

      final service = HiveToSqliteMigrationService(
        HiveToSqliteMigrationDecision(
          needsMigration: true,
          appDataDir: tempDir,
          sqliteFile: File('${tempDir.path}/canary.db'),
  ...
  test('attempt counter clears after successful migration', () async {
    ...
    await Directory('${tempDir.path}/canary.db').delete(recursive: true);
  ...
  test(
    ...
    () async {
      final live = File('${tempDir.path}/canary.db')
        ..writeAsStringSync('old-placeholder');
      final temp = File('${tempDir.path}/canary.db.migrating')
