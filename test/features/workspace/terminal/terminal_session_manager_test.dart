...
import 'package:Canary/features/workspace/terminal/osc_1337.dart';
...
void main() {
  ...
  test('intercepts OSC 1337 CanaryOpenURL and does not render it', () async {
    final runtime = FakeWorkspaceRuntime();
    final manager = managerWithLock((_) {});
    addTearDown(() async {
      await manager.closeAll();
    ...
    runtime.lastPty!.emitString(
      'hello\x1b]1337;CanaryOpenURL=https://example.com/path\x07world',
    );
    await flush();
    ...
    expect(text, isNot(contains('CanaryOpenURL')));
  ...
  test('intercepts OSC 1337 terminated with ST across chunks', () async {
    ...
    runtime.lastPty!.emit(
      utf8.encode('\x1b]1337;CanaryOpenURL=canary://workspace/a.txt'),
    );
    ...
    expect(text, isNot(contains('CanaryOpenURL')));
    expect(urls, [Uri.parse('canary://workspace/a.txt')]);
  });
  ...
  test('Osc1337Interceptor holds incomplete sequences', () {
    ...
    final first = interceptor.process(
      utf8.encode('pre\x1b]1337;CanaryOpenURL=https://x'),
    );
  ...
  test(
    'Osc1337Interceptor still intercepts CanaryOpenURL after OSC give-up',
    () {
      final urls = <Uri>[];
      final interceptor = Osc1337Interceptor(onUrl: urls.add);
      ...
      final filtered = interceptor.process(
        utf8.encode('\x1b]1337;CanaryOpenURL=canary://workspace/a.txt\x07'),
      );
      expect(utf8.decode(filtered), isNot(contains('CanaryOpenURL')));
      expect(urls, [Uri.parse('canary://workspace/a.txt')]);
    },
  );
  ...
  test('Osc1337Interceptor holds trailing ESC across chunks', () {
    ...
    final second = interceptor.process(
      utf8.encode(']1337;CanaryOpenURL=canary://workspace/a.txt\x07post'),
    );
    expect(utf8.decode(second), 'post');
    expect(urls, [Uri.parse('canary://workspace/a.txt')]);
  });
