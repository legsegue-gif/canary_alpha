...
import 'package:Canary/features/workspace/widgets/preview/code_file_preview.dart';
...
class _RecordingResolver extends FileLinkResolver {
  ...
  final List<CanaryLink> calls = <CanaryLink>[];
  File? result;
...
void main() {
  ...
  setUp(() {
    tempDir = Directory.systemTemp.createTempSync('canary_preview_');
  });
  ...
  testWidgets('canary://workspace tap calls the injected resolver', (
    tester,
  ) async {
        ...
        child: MaterialApp(
          ...
          home: const Scaffold(
            body: MarkdownWithCodeHighlight(
              text: '[foo](canary://workspace/foo.txt)',
            ),
          ),
        ),
      ),
    ...
    expect(resolver.calls.single.kind, CanaryLinkKind.workspaceFile);
