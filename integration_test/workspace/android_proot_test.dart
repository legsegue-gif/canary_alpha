...
import 'package:Canary/core/services/sandbox/android_proot_runtime.dart';
...
import 'package:Canary/core/services/sandbox/mirror_speed_test.dart';
import 'package:Canary/core/services/sandbox/mobile_workspace_bootstrap.dart';
...
import 'package:Canary/features/workspace/terminal/terminal_session_manager.dart';
import 'package:Canary/utils/app_directories.dart';
import 'package:Canary/utils/mcp_structured_image.dart';
...
void main() {
  ...
  testWidgets('3 environment install reaches ready', (tester) async {
    ...
    expect(
      File(p.join(h.installer.rootfsDir.path, kCanaryVersionFile)).existsSync(),
      isTrue,
    );
    _log(
...
class _Harness {
  ...
  Future<void> ensureReady() async {
    final versionFile = File(
      p.join(installer.rootfsDir.path, kCanaryVersionFile),
    );
...
Future<BusinessPreferences> _openPreferences() async {
  ...
  if (await appDb.exists()) {
    try {
      ...
      _log('PREFS existing canary.db failed: $error\n$stack');
    }
  } else {
  ...
  final isolated = File(
    p.join(
      ...
      'canary-android-proot-it-${DateTime.now().millisecondsSinceEpoch}.db',
    ),
  );
  final prefs = BusinessPreferences(
...
Future<void> _assertPipMirrorRoundTrip(
  ...
  if (await _guestExists(h, '/etc/pip.conf')) {
    ...
    await h.run(
      'rm -f /etc/pip.conf /etc/pip.conf.bak /etc/pip.conf.canary-created',
    );
  }
  expect(
  ...
  expect(await _guestExists(h, '/etc/pip.conf.canary-created'), isTrue);
  ...
  expect(await _guestExists(h, '/etc/pip.conf.canary-created'), isFalse);
  expect(await _guestExists(h, '/etc/pip.conf.bak'), isFalse);
}
...
Future<void> _assertAptMirrorRoundTrip(
  ...
  expect(await _guestExists(h, '$sources.canary-created'), isFalse);
