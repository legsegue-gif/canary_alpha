...
import 'package:Canary/core/services/workspace/output_buffer.dart';
...
void main() {
  ...
  setUp(() async {
    ...
    hostRoot = await Directory.systemTemp.createTemp('canary_macos_ws_');
...
Future<void> _cwdAndEnv(DesktopProcessRuntime runtime, Directory cwd) async {
  ...
  final envEvents = await _collect(
    ...
    _req(
      runId: 'env',
      command: r'echo $CANARY_TEST',
      cwd: cwd.path,
      env: const <String, String>{'CANARY_TEST': 'canary-macos-ok'},
    ),
  );
  expect(utf8.decode(_stdout(envEvents)), contains('canary-macos-ok'));
}
...
Future<void> _toolsEndToEnd({
  ...
  final climb = p.relative(
    p.join(Directory.current.path, 'canary_macos_outside.txt'),
    from: hostRoot.path,
  );
  ...
  final absOutside = p.join(Directory.current.path, 'canary_macos_abs.txt');
  addTearDown(() {
