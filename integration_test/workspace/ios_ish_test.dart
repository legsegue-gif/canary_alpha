...
import 'package:Canary/core/services/sandbox/ios_ish_runtime.dart';
...
import 'package:Canary/core/services/workspace/host_file_tools.dart';
...
void main() {
  ...
  setUpAll(() async {
    ...
    final tmp = await Directory.systemTemp.createTemp('canary_ios_ish_');
  ...
  group('iOS iSH workspace', () {
    iosTest('cold boot restores external mounts before commands', (
      ...
      try {
        ...
        if (!const bool.fromEnvironment('CANARY_TEST_IMPLICIT_BOOT')) {
          await channel.boot();
        }
...
for base in /tmp /workspace; do
  dir=$(mktemp -d "$base/canary-cp-XXXXXX")
