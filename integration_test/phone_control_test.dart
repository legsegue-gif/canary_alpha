...
// Run on an unlocked Android test device with Canary's accessibility service
...
void main() {
  ...
  setUpAll(() async {
    ...
    expect(
      ...
      isTrue,
      reason:
          'Enable Canary phone control in Android Accessibility settings on this test device.',
    );
    await Future<void>.delayed(const Duration(milliseconds: 500));
  });
  ...
  testWidgets('phone control reads and operates real Android windows', (
    ...
    expect(snapshot['package_name'], 'app.canary.client');
    ...
    await call({'action': 'open_app', 'package_name': 'app.canary.client'});
    snapshot = await waitForPackage((name) => name == 'app.canary.client');
    expect(snapshot['package_name'], 'app.canary.client');
  });
}
