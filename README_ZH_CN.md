...
<p>
  ...
  <a href="https://qm.qq.com/q/eAqLIYo4LK"><img src="https://img.shields.io/badge/QQ%20%E7%BE%A4-0366CC?style=for-the-badge&amp;logo=qq&amp;logoColor=white" alt="QQ 群" /></a>
</p>

[官网](https://github.com/legsegue-gif/canary_alpha) · [使用手册](https://github.com/legsegue-gif/canary_alpha) · [下载](#-下载) · [反馈问题](https://github.com/legsegue-gif/canary_alpha/issues)
...
Canary 是基于 Flutter 构建的跨平台 LLM 客户端，支持 Android、iOS、macOS、Windows 和 Linux。你可以填入自己的 API Key，也可以直接登录已支持的订阅账号，在同一个应用里使用 OpenAI、Gemini、Claude、DeepSeek、OpenRouter 以及任何 OpenAI 兼容服务。

Canary 不只是聊天。模型可以联网搜索、调用 MCP 服务器、按技能完成任务，并记住对你重要的信息。为对话绑定**工作区**后，模型还能读写文件、执行命令：在手机上运行于 Linux 沙盒中，在电脑上直接使用本机 Shell。

对话、设置和文件都保存在本地。Canary 没有自己的账号体系，需要时可以备份到 WebDAV 或 S3 兼容存储。
...
<tr>
<td width="180" align="center" valign="middle">
  <a href="https://api.muteki.site/register?aff=canary&promo=canary"><img src="docs/sponsors/marucode.jpg" alt="MaruCode" width="150"></a>
</td>
<td valign="middle"><b><a href="https://api.muteki.site/register?aff=canary&promo=canary">MaruCode</a></b> 是一家偶尔做做慈善的小破站 API，自营号池，主要提供 Codex、Claude Code、GPT Image 等主流模型，支持 Websocket 协议，明码标价(Codex 0.25x, CC 1.5x)，透明汇率(1:1)，<a href="https://api.muteki.site/register?aff=canary&promo=canary">新用户注册送 2 刀</a>。<a href="https://images-2.muteki.site">生图工作台🖼️</a></td>
</tr>
</table>

如果 Canary 对你有帮助，也欢迎通过[微信赞赏](docx/sponsor.jpg)支持项目。
...
| 平台 | 获取方式 | 安装包 | 系统要求 |
| --- | --- | --- | --- |
| iOS / iPadOS | [App Store](https://github.com/legsegue-gif/canary_alpha) · [TestFlight](https://github.com/legsegue-gif/canary_alpha)（测试版） | App Store；[Releases](https://github.com/legsegue-gif/canary_alpha/releases/latest) 另提供未签名 IPA | iOS 15.0 及以上 |
| Android | [GitHub Releases](https://github.com/legsegue-gif/canary_alpha/releases/latest) | APK（`arm64-v8a`、`armeabi-v7a`、`x86_64`） | Android 7.0 及以上 |
| macOS | [GitHub Releases](https://github.com/legsegue-gif/canary_alpha/releases/latest) | DMG | macOS 11.0 及以上，支持 Apple 芯片与 Intel |
| Windows | [GitHub Releases](https://github.com/legsegue-gif/canary_alpha/releases/latest) | 安装程序（`setup.exe`）或免安装 ZIP | Windows 10 / 11 |
| Linux | [GitHub Releases](https://github.com/legsegue-gif/canary_alpha/releases/latest) | AppImage、DEB、RPM、tar.gz | x86_64 |
| HarmonyOS | [canary-ohos](https://github.com/legsegue-gif/canary_alpha) | 在独立仓库中维护 | — |

官网的[下载页](https://github.com/legsegue-gif/canary_alpha/releases/latest)提供同样的安装包。
...
[使用手册](https://github.com/legsegue-gif/canary_alpha)详细介绍了添加模型、记忆、世界书等功能的用法。
...
- **联网搜索**：支持 24 种搜索服务，包括 Bing、DuckDuckGo、SearXNG、Brave、Exa、Tavily、Jina、Perplexity、Serper、Firecrawl、You.com、LinkUp、Parallel、Querit、TinyFish、AnySearch、Grok、Ollama、博查、秘塔、智谱、豆包、阶跃星辰和 Canary。多个 API Key 自动轮换，回答附带引用来源。
...
- **手机端**：生成可在后台持续进行，并在完成时通知；iOS 支持实时活动，Android 支持实时通知和任务悬浮窗。可以从其他应用分享文本和文件到 Canary，Android 上还能通过文本选择菜单直接发送选中的文字。
- **桌面端**：多栏布局、可自定义的快捷键（包括显示/隐藏 Canary 的全局快捷键）、系统托盘、拖拽添加附件，并在重新启动后恢复窗口大小与位置。
...
| 定时任务 | ✓ | — | ✓ |
| 后台生成 | 常驻通知、实时通知、任务悬浮窗 | 增强后台运行、实时活动 | Canary 运行期间 |
...
- **问题反馈与功能建议**：请使用 [Issue 模板](https://github.com/legsegue-gif/canary_alpha/issues/new/choose)。
...
- **交流讨论**：欢迎加入 [Discord](https://discord.gg/5BXdVPyen) 或 [QQ 群](https://qm.qq.com/q/eAqLIYo4LK)。

## 🙏 致谢

- [RikkaHub](https://github.com/re-ovo/rikkahub)：Canary 的界面设计深受其优美而实用的设计启发。
...
- Canary 依赖的所有开源软件包，详见 [`pubspec.yaml`](pubspec.yaml)。
...
Canary 基于 [GNU Affero General Public License v3.0](LICENSE) 开源。
