...
# ============================================================================
# Alpine Linux aarch64 rootfs for the Canary iOS Workspace (iSH fakefs)
# ============================================================================
...
install_apks() {
    ...
    for mirror in "${ALPINE_MIRRORS[@]}"; do
        ...
        if "$ISH" -f "$FAKEFS_OUT" /bin/sh -c "$(cat "$SCRIPT_DIR/overlay/usr/local/bin/canary-repair-rootfs")"; then
            installed=1
            break
        fi
...
PY
    ...
    # Shell defaults live in overlay/etc/profile and profile.d/canary.sh.
...
main() {
    echo ""
    echo "============================================================"
    echo "  Canary Alpine aarch64 fakefs rootfs"
