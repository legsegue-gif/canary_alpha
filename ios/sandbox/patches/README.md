...
(`64933c9d6b8890fb9f0fe5a581e78fdad9ea5256`, legsegue-gif/solos-ish).
...
patches. On every cold boot, `CanaryISHKernel` applies its manifest through the
guest VFS before launching processes, including in existing environments.
Canary's own `overlay/` is applied afterward. No environment reset is required.
...
The shared `overlay/usr/local/bin/canary-repair-rootfs` transaction also repairs
