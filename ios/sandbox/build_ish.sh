...
# ============================================================================
# iSH-ARM64 static library build for the Canary iOS Workspace sandbox
# ============================================================================
# Builds libish.a / libish_emu.a / libfakefs.a (arm64) from solos-ish
# (a modified ish-arm64), which emulates an aarch64 Linux userland inside the app
...
#
# Canary patches (ios/sandbox/patches/*.patch) are applied after fetch_ish
...
ISH_REPO="https://github.com/legsegue-gif/solos-ish.git"
ISH_SHA="64933c9d6b8890fb9f0fe5a581e78fdad9ea5256"
...
main() {
    ...
    echo ""
    echo "============================================================"
    echo "  Canary iOS Workspace: iSH-ARM64 static library builder"
    ...
    if [ ! -f "$OUTPUT_DIR/.canary-ish-build" ] || [ "$(cat "$OUTPUT_DIR/.canary-ish-build")" != "$fingerprint" ]; then
    ...
    for sdk in $sdks; do
        printf '%s\n' "$fingerprint" > "$(sdk_output_dir "$sdk")/.canary-ish-build"
    done
    printf '%s\n' "$fingerprint" > "$OUTPUT_DIR/.canary-ish-build"
    print_summary
}
