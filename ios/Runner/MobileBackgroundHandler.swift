...
struct BackgroundGenerationTask {
  ...
  init(_ map: [String: Any]) {
    ...
    title = String((map["title"] as? String ?? "Canary").prefix(120))
...
final class MobileBackgroundHandler: NSObject, CLLocationManagerDelegate {
  ...
  private var lastError = UserDefaults.standard.string(forKey: "canary.background.lastError") ?? ""
  private var dartReady = false
  private static let pendingConversationKey = "canary.background.pendingConversation"
  ...
  func configure(messenger: FlutterBinaryMessenger) {
    ...
    if UserDefaults.standard.bool(forKey: "canary.background.hadTasks") {
      recordError("previous_process_terminated")
    }
    UserDefaults.standard.set(false, forKey: "canary.background.hadTasks")
  ...
  func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "sync":
      ...
      enqueue { handler in
        ...
        UserDefaults.standard.set(!handler.tasks.isEmpty, forKey: "canary.background.hadTasks")
  ...
  func receive(_ url: URL) -> Bool {
    guard url.scheme == "canary", url.host == "conversation" else { return false }
  ...
  private func requestPermission(_ permission: String, result: @escaping FlutterResult) {
    guard UIApplication.shared.applicationState == .active else {
      result(FlutterError(code: "foreground_required", message: "Open Canary to request permission.", details: nil)); return
    }
    switch permission {
    case "notifications":
  ...
  private func evaluateRuntime() {
    ...
    if needed && assertion == .invalid && !assertionExpired {
      assertion = UIApplication.shared.beginBackgroundTask(withName: "CanaryGeneration") { [weak self] in
        guard let self else { return }
  ...
  private func startSilentAudio() {
    ...
    do {
      ...
      guard let format = AVAudioFormat(standardFormatWithSampleRate: 44100, channels: 1),
            let buffer = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: 44100) else { throw NSError(domain: "CanaryAudio", code: 1) }
  ...
  private func updateActivity(force: Bool) async {
    ...
    if #available(iOS 16.1, *) {
      var existing = activity as? Activity<CanaryGenerationActivityAttributes>
      ...
      if !didCleanActivities || !enabled("liveActivitiesEnabled") || !inBackground {
        for other in Activity<CanaryGenerationActivityAttributes>.activities where other.id != existing?.id {
      ...
      if let existing, isRunning(existing) {
        ...
        do {
          let attributes = CanaryGenerationActivityAttributes(groupId: UUID().uuidString)
          let next: Activity<CanaryGenerationActivityAttributes>
          if #available(iOS 16.2, *) {
          ...
          activityObserver = Task { @MainActor [weak self] in
            for await state in next.activityStateUpdates {
              ...
              if state == .dismissed || state == .ended {
                if (self.activity as? Activity<CanaryGenerationActivityAttributes>)?.id == next.id {
  ...
  @available(iOS 16.1, *)
  private func isRunning(_ activity: Activity<CanaryGenerationActivityAttributes>) -> Bool {
    if #available(iOS 16.2, *) {
  ...
  @available(iOS 16.1, *)
  private func contentState(task: BackgroundGenerationTask, finished: Bool) -> CanaryGenerationActivityAttributes.ContentState {
    CanaryGenerationActivityAttributes.ContentState(
      ...
      staleMessage: labels["stale"] ?? "Open Canary to check the task.")
  }

  @available(iOS 16.1, *)
  private func end(_ target: Activity<CanaryGenerationActivityAttributes>,
                   state: CanaryGenerationActivityAttributes.ContentState?, immediately: Bool) async {
  ...
  private func returnStatus(_ result: @escaping FlutterResult) {
    ...
    if Self.liveActivitiesSupported, #available(iOS 16.1, *) {
      ...
      if let activity = activity as? Activity<CanaryGenerationActivityAttributes> {
  ...
  private func clearCompletionNotifications() {
    UNUserNotificationCenter.current().getDeliveredNotifications { notifications in
      let ids = notifications.filter { $0.request.content.threadIdentifier == "canary.chat-completion" }.map { $0.request.identifier }
  ...
  private func recordError(_ error: String) {
    lastError = error
    UserDefaults.standard.set(error, forKey: "canary.background.lastError")
    NSLog("Canary background: %@", error)
  }
