...
final class WorkspaceDirectoryAccess: NSObject, UIDocumentPickerDelegate {
  private let queue = DispatchQueue(label: "app.canary.client.workspace.directories", qos: .userInitiated)
  ...
  func resolve(token: String, result: @escaping FlutterResult) {
    perform(result) {
      ...
      // follows moves performed in Files while Canary remains running.
  ...
  private func unavailable() -> NSError {
    NSError(domain: "CanaryWorkspaceDirectory", code: 1,
