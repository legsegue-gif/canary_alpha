...
#include "CanaryISHCrashGuards.h"
...
static void canary_jit_crash_handler(int sig, siginfo_t *info, void *ctx) {
...
void CanaryISHInstallCrashGuards(void) {
    ...
    sa.sa_sigaction = canary_jit_crash_handler;
...
static void canary_die_handler(const char *msg) {
    char buf[4096];
    int len = snprintf(buf, sizeof(buf), "\n=== iSH fatal: %s ===\n", msg);
    if (len > 0) write(STDERR_FILENO, buf, (size_t)len);
    park_thread_forever();
...
void CanaryISHInstallDieGuard(void) {
    die_handler = canary_die_handler;
}
