...
final class WorkspacePlugin: NSObject, FlutterStreamHandler {
  ...
  private let queue = DispatchQueue(label: "app.canary.client.workspace", qos: .userInitiated)
  ...
  static func register(messenger: FlutterBinaryMessenger, presenter: UIViewController) -> WorkspacePlugin {
    ...
    let kernel = CanaryISHKernel.shared()
  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "setExternalMounts":
      ...
      queue.async {
        if !NSArray(array: mounts).isEqual(to: self.externalMounts) {
          CanaryISHExecutor.cancelAllInterrupted(true)
          CanaryISHKernel.shared().ptyCloseAll()
          let error = CanaryISHKernel.shared().reconcileExternalBinds(mounts)
          if error < 0 {
            let rollback = CanaryISHKernel.shared().reconcileExternalBinds(self.externalMounts)
            self.externalMountsApplied = rollback >= 0 && CanaryISHKernel.shared().isBooted
            self.complete(result, Self.mountError(error))
            return
          }
          self.externalMounts = mounts
          self.externalMountsApplied = CanaryISHKernel.shared().isBooted
        }
    ...
    case "stdinWrite":
      ...
      DispatchQueue.global(qos: .userInitiated).async {
        let ok = CanaryISHExecutor.writeStdin(data.data, runId: runId)
  ...
  private func exec(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    queue.async { [weak self] in
      ...
      let started = CanaryISHExecutor.startCommand(
        command,
        runId: runId,
  ...
  private func cancel(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    result(CanaryISHExecutor.cancelRunId(runId))
  }
  ...
  private func ptyOpen(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    queue.async { [weak self] in
      ...
      let pid = CanaryISHKernel.shared().ptyOpenSession(
        sessionId,
        binds: self.commandBinds(binds),
        cwd: cwd,
        env: env,
  ...
  private static func ptyOpenError(_ code: Int32) -> FlutterError {
    let reason: String
    switch code {
    case CanaryISHPtyOpenError.notBooted.rawValue:
      reason = "kernel not booted"
    case CanaryISHPtyOpenError.badSessionId.rawValue:
      reason = "empty sessionId"
    case CanaryISHPtyOpenError.sessionExists.rawValue:
      reason = "sessionId already open"
    case CanaryISHPtyOpenError.environmentTooLarge.rawValue:
      reason = "environment variables exceed the iOS sandbox limit of 128 KiB (UTF-8); reduce their total size"
    case CanaryISHPtyOpenError.invalidEnvironment.rawValue:
  ...
  private func ptyWrite(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    CanaryISHKernel.shared().ptyWriteSession(sessionId, data: data)
    result(nil)
  }
  ...
  private func ptyResize(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    let cols = (args["cols"] as? NSNumber)?.int32Value ?? 80
    let rows = (args["rows"] as? NSNumber)?.int32Value ?? 24
    CanaryISHKernel.shared().ptyResizeSession(sessionId, cols: cols, rows: rows)
    result(nil)
  }
  ...
  private func ptyClose(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    if let sessionId = args["sessionId"] as? String {
      CanaryISHKernel.shared().ptyCloseSession(sessionId)
    }
    result(nil)
  }
  ...
  private static func mountError(_ code: Int32) -> FlutterError {
    if code == CanaryISHMountTargetOccupied {
      return FlutterError(
