...
#import "CanaryISHKernel.h"
#import "CanaryISHCrashGuards.h"
#import "CanaryISHExecutor.h"
#import "CanaryISHEnvironment.h"
#import "CanaryISHFilesystem.h"
#import "CanaryISHCompat.h"
...
NSNotificationName const CanaryISHProcessExitedNotification = @"CanaryISHProcessExited";

static NSString *const kDnsSubdir = @"CanaryWorkspace/dns";
...
static int canary_console_write(struct tty *tty, const void *buf, size_t len, bool blocking) {
    (void)tty;
    (void)buf;
    (void)blocking;
    return (int)len;
...
static int canary_console_init(struct tty *tty) {
    (void)tty;
    return 0;
}

static void canary_console_cleanup(struct tty *tty) {
    (void)tty;
}

static struct tty_driver_ops canary_console_ops = {
    .init = canary_console_init,
    .write = canary_console_write,
    .cleanup = canary_console_cleanup,
};

DEFINE_TTY_DRIVER(canary_console_driver, &canary_console_ops, TTY_CONSOLE_MAJOR, 8);

#pragma mark - PTY driver

static int canary_pty_write(struct tty *tty, const void *buf, size_t len, bool blocking);
static int canary_pty_init(struct tty *tty) {
    (void)tty;
    return 0;
}
static void canary_pty_cleanup(struct tty *tty) {
    (void)tty;
}

static struct tty_driver_ops canary_pty_ops = {
    .init = canary_pty_init,
    .write = canary_pty_write,
    .cleanup = canary_pty_cleanup,
};

static struct tty_driver canary_pty_driver = {.ops = &canary_pty_ops};

static bool canary_reverse_context_path(const char *host, char *out, size_t size) {
    uint64_t context = current && current->group ? current->group->fs_context : 0;
    return CanaryISHReversePath(host, context, out, size);
}

#pragma mark - Session / bind state

@interface CanaryISHPtySession : NSObject
...
@implementation CanaryISHPtySession
@end

@interface CanaryISHKernel ()
...
- (void)flushPtyOutput:(CanaryISHPtySession *)session;
@end
...
static int canary_decode_wait_status(int status) {
...
static void canary_handle_process_exit(struct task *task, int code) {
    if (task->parent != NULL && task->parent->parent != NULL)
        return;
    pid_t pid = task->pid;
    int decoded = canary_decode_wait_status(code);
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter]
            postNotificationName:CanaryISHProcessExitedNotification
                          object:nil
                        userInfo:@{@"pid": @(pid), @"code": @(decoded)}];
        [[CanaryISHKernel shared] noteGuestExitWithPid:pid code:decoded];
    });
}
...
static int canary_pty_write(struct tty *tty, const void *buf, size_t len, bool blocking) {
    ...
    if (len == 0) return 0;
    @autoreleasepool {
        [[CanaryISHKernel shared] queuePtyBytes:buf length:len ttyNum:tty->num];
    }
    return (int)len;
}

@implementation CanaryISHKernel {
    BOOL _isBooted;
    NSString *_rootPath;
    NSString *_dataPath;
    ...
    NSMutableDictionary<NSString *, CanaryISHPtySession *> *_ptyBySession;
    NSMutableDictionary<NSNumber *, CanaryISHPtySession *> *_ptyByPid;
    NSMutableDictionary<NSNumber *, CanaryISHPtySession *> *_ptyByTtyNum;
    NSLock *_ptyLock;
...
+ (instancetype)shared {
    static CanaryISHKernel *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[CanaryISHKernel alloc] init];
    });
    return instance;
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _spawnQueue = dispatch_queue_create("app.canary.client.workspace.ish.spawn", DISPATCH_QUEUE_SERIAL);
...
- (int)bootWithRootPath:(NSString *)rootPath {
    if (_isBooted) {
        NSLog(@"CanaryISHKernel: already booted");
        return 0;
    }

    int err;
    CanaryISHInstallCrashGuards();
    CanaryISHInstallDieGuard();

    _rootPath = rootPath;
    _dataPath = [rootPath stringByAppendingPathComponent:@"data"];
    err = mount_root(&fakefs, _dataPath.fileSystemRepresentation);
    if (err < 0) {
        NSLog(@"CanaryISHKernel: mount_root failed: %d", err);
        _rootPath = nil;
        _dataPath = nil;
        return err;
    }
    ...
    if (realpath(_dataPath.fileSystemRepresentation, canonical_data_path) != NULL) {
        ...
        NSLog(@"CanaryISHKernel: realpath(data) failed (errno=%d)", errno);
    }

    err = become_first_process();
    if (err < 0) {
        NSLog(@"CanaryISHKernel: become_first_process failed: %d", err);
        return err;
    }
    ...
    fakefs_set_path_translate_hook(CanaryISHTranslatePath);
    fakefs_set_path_reverse_hook(canary_reverse_context_path);
    exit_hook = canary_handle_process_exit;

    tty_drivers[TTY_CONSOLE_MAJOR] = &canary_console_driver;
    set_console_device(TTY_CONSOLE_MAJOR, 1);
    err = create_stdio("/dev/console", TTY_CONSOLE_MAJOR, 1);
    if (err < 0) {
        NSLog(@"CanaryISHKernel: create_stdio failed: %d (non-fatal)", err);
    }

    _isBooted = YES;
    NSLog(@"CanaryISHKernel: kernel initialized");
    return 0;
}
...
- (void)applyRootfsPatchBundle {
    ...
    if (manifest == nil) {
        NSLog(@"CanaryISHKernel: RootfsPatch manifest missing from bundle");
        return;
    }
...
- (void)applyBundleOverlay {
    ...
    if (overlayURL == nil) {
        NSLog(@"CanaryISHKernel: overlay folder missing from bundle");
        return;
    }
...
- (void)writeGuestFile:(NSString *)guestPath fromHostURL:(NSURL *)hostURL {
    ...
    if (contents == nil) {
        NSLog(@"CanaryISHKernel: overlay read failed: %@", hostURL.path);
        return;
    }
    ...
    if (IS_ERR(fd)) {
        NSLog(@"CanaryISHKernel: overlay open %@ failed: %ld", guestPath, PTR_ERR(fd));
        return;
    }
    ...
    while (offset < contents.length) {
        ...
        if (written <= 0) {
            NSLog(@"CanaryISHKernel: overlay write %@ failed: %zd", guestPath, written);
            break;
        }
...
- (void)mountDnsConfig {
    ...
    if (err < 0) {
        NSLog(@"CanaryISHKernel: DNS bind mount failed (%d) — writing through VFS", err);
...
- (void)refreshDnsConfig {
    ...
    if (![content writeToFile:_dnsHostPath atomically:YES encoding:NSUTF8StringEncoding error:&error]) {
        NSLog(@"CanaryISHKernel: DNS refresh write failed: %@", error);
    }
}

static void CanaryDnsReachabilityChanged(SCNetworkReachabilityRef target,
                                         SCNetworkReachabilityFlags flags,
                                         void *info) {
    (void)target;
    (void)flags;
    @autoreleasepool {
        CanaryISHKernel *kernel = (__bridge CanaryISHKernel *)info;
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)startDnsWatcher {
    ...
    if (!SCNetworkReachabilitySetCallback(reachability, CanaryDnsReachabilityChanged, &context) ||
...
- (void)setUpUnixSocketPrefix {
    ...
    if (prefixLen + kSuffixWorstCase >= kSunPathMax) {
        NSLog(@"CanaryISHKernel: unix socket prefix too long — keeping default");
        return;
    }
...
- (int)bindMountPath:(NSString *)linuxPath toHostPath:(NSString *)hostPath readOnly:(BOOL)readOnly {
    ...
    int targetError = CanaryISHValidateMountTarget(_dataPath, linuxPath);
    ...
    if (![fm fileExistsAtPath:hostPath isDirectory:&isDir]) {
        ...
        if (![fm createDirectoryAtPath:hostPath withIntermediateDirectories:YES attributes:nil error:&error]) {
            NSLog(@"CanaryISHKernel: host mkdir %@ failed: %@", hostPath, error);
            return -1;
        }
    }
    struct task *prev = current;
    ...
    if (err < 0) {
        NSLog(@"CanaryISHKernel: bindMount %@ -> %@ failed: %d", linuxPath, hostPath, err);
        return err;
    }
...
- (uint64_t)filesystemContextForBinds:(NSArray<NSDictionary<NSString *, id> *> *)binds {
    NSData *data = CanaryISHCreateFilesystem(binds);
    if (!data) return 0;
...
- (int)reconcileExternalBinds:(NSArray<NSDictionary<NSString *, id> *> *)binds {
    ...
    for (NSDictionary<NSString *, id> *bind in binds) {
        int err = CanaryISHValidateMountTarget(_dataPath, bind[@"guest"]);
        if (err < 0) return err;
    }
...
- (int)ptyOpenSession:(NSString *)sessionId
                binds:(NSArray<NSDictionary<NSString *, id> *> *)binds
                 ...
                 cols:(int)cols
                 rows:(int)rows {
    if (!_isBooted) return CanaryISHPtyOpenErrorNotBooted;
    if (sessionId.length == 0) return CanaryISHPtyOpenErrorBadSessionId;

    [_ptyLock lock];
    BOOL exists = _ptyBySession[sessionId] != nil;
    [_ptyLock unlock];
    if (exists) {
        NSLog(@"CanaryISHKernel: pty session %@ is already open", sessionId);
        return CanaryISHPtyOpenErrorSessionExists;
    }
    ...
    CanaryISHEnvironmentError environmentError;
    NSData *environmentData = CanaryISHEncodeEnvironment(merged, &environmentError);
    if (environmentData == nil) {
        return environmentError == CanaryISHEnvironmentErrorTooLarge
            ? CanaryISHPtyOpenErrorEnvironmentTooLarge
            : CanaryISHPtyOpenErrorInvalidEnvironment;
    }
    ...
    [self performOnSpawnQueue:^{
        ...
        @try {
            ...
            struct tty *tty = pty_open_fake(&canary_pty_driver);
            if (IS_ERR(tty)) {
            ...
            CanaryISHPtySession *session = [[CanaryISHPtySession alloc] init];
            ...
            __weak CanaryISHPtySession *outputSession = session;
            ...
            dispatch_source_set_event_handler(session.outputTimer, ^{
                CanaryISHPtySession *active = outputSession;
...
- (void)ptyWriteSession:(NSString *)sessionId data:(NSData *)data {
    [_ptyLock lock];
    CanaryISHPtySession *session = _ptyBySession[sessionId];
...
- (void)ptyCloseSession:(NSString *)sessionId {
    ...
    [_ptyLock unlock];
    if (!session) return;
    [CanaryISHExecutor killGuestPid:session.pid groupId:session.pgid];
}
...
- (void)queuePtyBytes:(const void *)bytes length:(size_t)length ttyNum:(int)ttyNum {
    ...
    [_ptyLock lock];
    CanaryISHPtySession *session = _ptyByTtyNum[@(ttyNum)];
...
static NSArray<NSData *> *canary_take_pty_output(CanaryISHPtySession *session) {
...
- (void)flushPtyOutput:(CanaryISHPtySession *)session {
    ...
    dispatch_source_set_timer(session.outputTimer, DISPATCH_TIME_FOREVER, DISPATCH_TIME_FOREVER, 0);
    NSArray<NSData *> *packets = canary_take_pty_output(session);
    NSString *sessionId = session.sessionId;
    CanaryISHPtyDataHandler handler = self.ptyDataHandler;
    [_ptyLock unlock];
...
- (void)noteGuestExitWithPid:(int)pid code:(int)code {
    ...
    NSString *sessionId = session.sessionId;
    CanaryISHPtyExitHandler handler = self.ptyExitHandler;
    CanaryISHPtyDataHandler dataHandler = self.ptyDataHandler;
    [_ptyLock unlock];
