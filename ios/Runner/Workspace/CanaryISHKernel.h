...
//  Runner
//
//  Objective-C wrapper around the embedded iSH-ARM64 kernel for the Canary
...
extern NSNotificationName const CanaryISHProcessExitedNotification;

typedef void (^CanaryISHPtyDataHandler)(NSString *sessionId, NSData *data);
typedef void (^CanaryISHPtyExitHandler)(NSString *sessionId, int exitCode);

/// Why -[CanaryISHKernel ptyOpenSession:...] refused before reaching the
...
typedef NS_ENUM(int, CanaryISHPtyOpenError) {
  CanaryISHPtyOpenErrorNotBooted = -1001,
  CanaryISHPtyOpenErrorBadSessionId = -1002,
  CanaryISHPtyOpenErrorSessionExists = -1003,
  CanaryISHPtyOpenErrorEnvironmentTooLarge = -1004,
  CanaryISHPtyOpenErrorInvalidEnvironment = -1005,
};

@interface CanaryISHKernel : NSObject
...
@property (nonatomic, copy, nullable) CanaryISHPtyDataHandler ptyDataHandler;
@property (nonatomic, copy, nullable) CanaryISHPtyExitHandler ptyExitHandler;
...
/// [CanaryISHPtyOpenError] when this wrapper refused the request.
///
