...
@interface CanaryISHRunContext : NSObject {
    int _stdoutReadEnd;
    int _stderrReadEnd;
    int _stdinPipe[2];
    int _stdoutPipe[2];
...
@property (nonatomic, copy) CanaryISHChunkHandler chunk;
@property (nonatomic, copy) CanaryISHDoneHandler done;
...
@implementation CanaryISHRunContext
...
@implementation CanaryISHExecutor

static NSMutableDictionary<NSNumber *, CanaryISHRunContext *> *_byPid;
static NSMutableDictionary<NSString *, CanaryISHRunContext *> *_byRunId;
static NSMutableSet<NSString *> *_cancelled;
static NSMutableSet<NSString *> *_queued;
static dispatch_queue_t _readerQueue;

+ (void)initialize {
    if (self == [CanaryISHExecutor class]) {
        ...
        _readerQueue = dispatch_queue_create("app.canary.client.workspace.ish.reader", DISPATCH_QUEUE_CONCURRENT);
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(processDidExit:)
                                                     name:CanaryISHProcessExitedNotification
                                                   object:nil];
    }
}
...
+ (BOOL)startCommand:(NSString *)command
       ...
       keepStdinOpen:(BOOL)keepStdinOpen
             started:(void (^)(void))started
               chunk:(CanaryISHChunkHandler)chunk
                done:(CanaryISHDoneHandler)done {
    ...
    [[CanaryISHKernel shared] performOnSpawnQueue:^{
        BOOL early = NO;
        @synchronized (_byPid) {
...
+ (void)spawnCommand:(NSString *)command
    ...
    void (^fail)(NSString *) = ^(NSString *reason) {
        ...
        NSLog(@"CanaryISHExecutor: %@", reason);
        done(@{
            @"exitCode": @(-1),
            @"timedOut": @NO,
            @"cancelled": @NO,
    ...
    if (![CanaryISHKernel shared].isBooted) {
    ...
    __attribute__((objc_precise_lifetime)) NSData *environmentData =
        CanaryISHEncodeEnvironment(merged, &environmentError);
    if (environmentData == nil) {
        NSString *reason = CanaryISHEnvironmentErrorMessage(environmentError);
        chunk(runId, YES, [[reason stringByAppendingString:@"\n"] dataUsingEncoding:NSUTF8StringEncoding]);
        fail(reason);
        return;
    }

    CanaryISHRunContext *ctx = [[CanaryISHRunContext alloc] init];
    ctx.runId = runId;
    ctx.chunk = chunk;
    ctx.done = done;
    ...
    if (keepStdinOpen) {
        if (CanaryISHCreateStdinPipe([ctx stdinPipe]) < 0) {
            [ctx closePipeEnds];
    ...
    uint64_t filesystem = [[CanaryISHKernel shared] filesystemContextForBinds:binds];
    if (!filesystem) {
        [ctx closePipeEnds];
    ...
    __weak CanaryISHRunContext *timedContext = ctx;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(timeout * NSEC_PER_SEC)),
                   dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
        CanaryISHRunContext *still;
        @synchronized (_byPid) {
            still = timedContext;
...
+ (void)finalizeContext:(CanaryISHRunContext *)ctx {
    @synchronized (ctx) {
...
+ (void)startReaderForPipe:(int)fd context:(CanaryISHRunContext *)ctx isStdErr:(BOOL)isStdErr {
...
+ (void)readPipe:(int)fd context:(CanaryISHRunContext *)ctx isStdErr:(BOOL)isStdErr {
...
static BOOL CanaryTaskIsDescendantOf(struct task *t, pid_t_ rootPid) {
    int hops = 0;
...
static void CanaryAddKillTarget(struct task *t, int *pids, struct task **ptrs, int *n, int max) {
    if (!t || *n >= max) return;
    for (int i = 0; i < *n; i++) {
        if (ptrs[i] == t) return;
    }
...
static void CanaryCollectChildren(struct task *task, int *pids, struct task **ptrs, int *n, int max) {
    if (!task || *n >= max) return;
    CanaryAddKillTarget(task, pids, ptrs, n, max);
    struct task *child;
    list_for_each_entry(&task->children, child, siblings) {
        CanaryCollectChildren(child, pids, ptrs, n, max);
    }
}
...
+ (void)killProcessGroup:(int)pid groupId:(pid_t_)knownPgid {
    ...
    if (rootTask) {
        CanaryCollectChildren(rootTask, targetPids, targetPtrs, &ntargets, kMaxKillTargets);
    }
    for (int i = 2; i < MAX_PID && ntargets < kMaxKillTargets; i++) {
        struct task *t = pid_get_task(i);
        if (!t) continue;
        BOOL byPgid = (pgid > 1 && t->group->pgid == pgid);
        BOOL byAncestry = CanaryTaskIsDescendantOf(t, (pid_t_)pid);
        if (!byPgid && !byAncestry) continue;
        CanaryAddKillTarget(t, targetPids, targetPtrs, &ntargets, kMaxKillTargets);
    }
    for (int i = 0; i < ntargets; i++) {
        send_signal(targetPtrs[i], SIGKILL_, info);
    }
    unlock(&pids_lock);
    NSLog(@"CanaryISHExecutor: kill root=%d pgid=%d targets=%d", pid, (int)pgid, ntargets);
    if (ntargets == 0) {
        free(targetPids);
        free(targetPtrs);
        return;
