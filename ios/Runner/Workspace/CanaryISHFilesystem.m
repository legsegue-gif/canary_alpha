...
typedef struct {
    char guest[16];
    char host[PATH_MAX];
} CanaryISHPathMapping;

NSData *CanaryISHCreateFilesystem(NSArray<NSDictionary<NSString *, id> *> *binds) {
    NSArray<NSString *> *roots = @[@"/workspace", @"/chat", @"/skills", @"/tmp"];
    CanaryISHPathMapping mappings[4] = {0};
...
static bool translate(const char *path, uint64_t context, char *out, size_t size, bool reverse) {
    ...
    const CanaryISHPathMapping *mappings = (const void *)(uintptr_t)context;
    const CanaryISHPathMapping *best = NULL;
    size_t bestLength = 0;
    for (int i = 0; i < 4; i++) {
...
bool CanaryISHTranslatePath(const char *guest, uint64_t context, char *out, size_t size) {
    return translate(guest, context, out, size, false);
}

bool CanaryISHReversePath(const char *host, uint64_t context, char *out, size_t size) {
