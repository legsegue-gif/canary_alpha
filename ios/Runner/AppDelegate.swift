...
private let backgroundRefreshIdentifier = "app.canary.client.background-generation.refresh"
private let backgroundProcessingIdentifier = "app.canary.client.background-generation.processing"
...
@objc class AppDelegate: FlutterAppDelegate {
  ...
  override func application(
    ...
    if url.scheme == "canary" && url.host == "oauth-return" {
      return true
    }
...
private final class IosBackgroundGenerationHandler {
  ...
  private func start(arguments: Any?, result: @escaping FlutterResult) {
    ...
    if args["liveActivityEnabled"] as? Bool ?? false {
      startLiveActivity(
        title: args["title"] as? String ?? "Canary",
  ...
  private func finish(arguments: Any?, result: @escaping FlutterResult) {
    let args = arguments as? [String: Any] ?? [:]
    let title = args["title"] as? String ?? "Canary"
  ...
  private func cancel(arguments: Any?, result: @escaping FlutterResult) {
    let args = arguments as? [String: Any] ?? [:]
    finishLiveActivity(
      title: liveActivityDisplayTitle.isEmpty ? "Canary" : liveActivityDisplayTitle,
      detail: args["detail"] as? String ?? ""
    )
    endBackgroundTask()
    resetGenerationOptions()
  ...
  private func beginBackgroundTask() {
    ...
    backgroundTask = UIApplication.shared.beginBackgroundTask(withName: "CanaryBackgroundGeneration") { [weak self] in
  ...
  private func scheduleBackgroundTasks() {
    ...
    do {
      ...
      NSLog("Canary background refresh schedule failed: \(error)")
    }
    ...
    do {
      ...
      NSLog("Canary background processing schedule failed: \(error)")
    }
  }
  ...
  private func showCompletionNotification(title: String, body: String) {
    ...
    let request = UNNotificationRequest(identifier: "canary.background-generation.\(Date().timeIntervalSince1970)", content: content, trigger: nil)
    UNUserNotificationCenter.current().add(request)
  }

  private func isLiveActivityActive() -> Bool {
    if #available(iOS 16.1, *) {
      return liveActivity as? Activity<CanaryGenerationActivityAttributes> != nil
    }
    return false
  }
    ...
    if #available(iOS 16.1, *) {
      ...
      do {
        if #available(iOS 16.2, *) {
          liveActivity = try Activity<CanaryGenerationActivityAttributes>.request(attributes: CanaryGenerationActivityAttributes(title: title), content: ActivityContent(state: state, staleDate: nil), pushType: nil)
        } else {
          liveActivity = try Activity<CanaryGenerationActivityAttributes>.request(attributes: CanaryGenerationActivityAttributes(title: title), contentState: state, pushType: nil)
        }
        startLiveActivityRefreshTimer()
      } catch {
        NSLog("Canary live activity start failed: \(error)")
        liveActivity = nil
      }
    }
  }
  ...
  private func markLiveActivityFinished(title: String, detail: String) {
    if #available(iOS 16.1, *), let activity = liveActivity as? Activity<CanaryGenerationActivityAttributes> {
      let finishedAt = Date()
      liveActivityDisplayTitle = title
      liveActivityDetail = detail
  ...
  private func refreshLiveActivity() {
    guard #available(iOS 16.1, *), let activity = liveActivity as? Activity<CanaryGenerationActivityAttributes> else { return }
    guard !liveActivityFinished else { return }
  ...
  private func liveActivityState(
    ...
    return CanaryGenerationActivityAttributes.ContentState(
