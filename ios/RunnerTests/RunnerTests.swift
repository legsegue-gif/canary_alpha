...
class IncomingShareHandlerTests: XCTestCase {
  ...
  func testOtherDeepLinksDoNotTriggerShareImport() {
    ...
    let handler = IosIncomingShareHandler()
    handler.register(messenger: messenger)
    for url in ["canary://oauth-return", "canary://conversation/chat", "https://share", "canary://share/file"] {
...
class RunnerTests: XCTestCase {
  ...
  func testActivityConversationLinkIsBufferedForColdStartAndConsumedOnce() async {
    let handler = MobileBackgroundHandler()
    XCTAssertFalse(handler.receive(URL(string: "canary://unrelated/chat")!))
    XCTAssertTrue(handler.receive(URL(string: "canary://conversation/chat-123")!))
  ...
  func testLiveActivityCreationCompletionAndRetryAreSerialized() async throws {
    ...
    XCTAssertFalse(Activity<CanaryGenerationActivityAttributes>.activities.contains { $0.activityState == .active })
    handler.prepareForTermination()
  }
  ...
  func testOrphanedLiveActivitiesAreEndedWhenRuntimeStartsDisabled() async throws {
    ...
    let state = CanaryGenerationActivityAttributes.ContentState(
    ...
    let orphan = try Activity.request(attributes: CanaryGenerationActivityAttributes(groupId: UUID().uuidString),
  ...
  func testSystemEndedActivityDoesNotReappearUntilANewRunStarts() async throws {
    ...
    let activity = try XCTUnwrap(Activity<CanaryGenerationActivityAttributes>.activities.first {
