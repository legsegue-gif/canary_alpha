package app.canary.client.workspace
...
import app.canary.client.IncomingShareHandler
...
class WorkspaceDocumentsProviderTest {
    ...
    fun setUp() {
        ...
        database = SQLiteDatabase.openOrCreateDatabase(File(appData, "canary.db"), null)
    ...
    fun exposesNamedWorkspacesWithoutFlutterOrDuplicateDatabase() {
        ...
        assertEquals(listOf("Canary"), provider.queryRoots(null).strings(Root.COLUMN_TITLE))
        assertEquals(listOf("工作区一", "另一个工作区"), children().strings(Document.COLUMN_DISPLAY_NAME))
        assertTrue(File(appData, "canary.db").exists())
    }
    ...
    fun rejectsAnOpenedDescriptorThatMovedOutsideTheWorkspace() {
        ...
        DocumentDescriptorOsShadow.openedPath = File(appData, "canary.db").path
    ...
    fun cursorReplacementKeepsFileAndRegistryNotificationsWorking() {
        ...
        try {
            repeat(3) {
                ...
                SharedPathFileObserverShadow.emit(appData, FileObserver.MODIFY, "canary.db-wal")
                shadowOf(Looper.getMainLooper()).idleFor(Duration.ofMillis(200))
    ...
    fun multipleDirectoriesShareRegistryWatchUntilTheLastCursorCloses() {
        ...
        try {
            first.close()
            first.close()
            SharedPathFileObserverShadow.emit(appData, FileObserver.MODIFY, "canary.db-shm")
            shadowOf(Looper.getMainLooper()).idleFor(Duration.ofMillis(200))
    ...
    fun excludesLinkedFoldersAndInvalidWorkspaceIds() {
        ...
        assertThrows(FileNotFoundException::class.java) { provider.queryDocument("workspace/linked/canary.db", null) }
    }
    ...
    fun rejectsTraversalUnknownWorkspacesAndCrossWorkspaceTrees() {
        ...
        for (id in listOf("workspace/one/../../../canary.db", "workspace/one/./ok.txt", "workspace/one//ok.txt", "workspace/one/a\u0000b", "workspace/unknown", "/etc/passwd")) {
    ...
    fun hidesEscapingSymlinksWhileAllowingInternalLinks() {
        ...
        assertThrows(FileNotFoundException::class.java) { provider.openDocument("workspace/one/private/canary.db", "r", null) }
    }
    ...
    fun refusesRedirectedWorkspaceRoots() {
        ...
        assertTrue(children().strings(Document.COLUMN_DOCUMENT_ID).isEmpty())
        assertThrows(FileNotFoundException::class.java) { provider.queryDocument("workspace/one/canary.db", null) }
    }
    ...
    fun missingOrUnsupportedDatabaseIsNotCreatedOrChanged() {
        database.close()
        val file = File(appData, "canary.db")
        file.delete()
        assertTrue(children().strings(Document.COLUMN_DOCUMENT_ID).isEmpty())
