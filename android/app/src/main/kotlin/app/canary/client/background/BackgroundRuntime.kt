package app.canary.client.background
...
import app.canary.client.MainActivity
import app.canary.client.CanaryApplication
import app.canary.client.R
...
class BackgroundRuntime(private val context: Context) {
    companion object {
        const val CHANNEL_ID = "canary_generation_status"
        const val NOTIFICATION_ID = 7
        const val NOTIFICATION_PERMISSION_REQUEST = 4301
        const val CONVERSATION_EXTRA = "canary.background.conversation"
        private const val PENDING_CONVERSATION = "pending_conversation"
    }

    private val prefs = context.getSharedPreferences("canary_background", Context.MODE_PRIVATE)
    ...
    fun beginScheduledRun(id: String) {
        ...
        blocked = false
        reconcileService()
        if (blocked) (context as CanaryApplication).scheduledTasks.stopAll("foreground_service_start_failed")
    }
    ...
    fun stopTasks(reason: String? = null) {
        ...
        (context as? CanaryApplication)?.scheduledTasks?.stopAll(reason ?: "cancelled")
    ...
    fun buildNotification(): Notification {
        ...
        val title = if (tasks.size > 1) "${tasks.size} ${label("tasks", "Tasks")}" else task?.title ?: "Canary"
    ...
    fun status(): Map<String, Any> {
        ...
        return mapOf(
            ...
            "completionChannelEnabled" to channelEnabled("canary_bg_chat_v2"),
    ...
    fun serviceFailed(message: String) {
        ...
        serviceStarting = false
        recordError(message)
        (context as? CanaryApplication)?.scheduledTasks?.stopAll(message)
        channel?.invokeMethod("statusChanged", null)
    }

    fun recordError(message: String) {
        prefs.edit().putString("last_error", message).apply()
        android.util.Log.w("CanaryBackground", message)
    }
            ...
            "channels", "ongoingChannel" -> if (Build.VERSION.SDK_INT >= 26) {
                ...
                intents += Intent(Settings.ACTION_CHANNEL_NOTIFICATION_SETTINGS)
                    ...
                    .putExtra(Settings.EXTRA_CHANNEL_ID,
                        if (destination == "ongoingChannel") CHANNEL_ID else "canary_bg_chat_v2")
            }
            "battery" -> {
    ...
    private fun clearCompletionNotifications() {
        ...
        for (entry in manager.activeNotifications) {
            if (Build.VERSION.SDK_INT >= 26 && entry.notification.channelId == "canary_bg_chat_v2") {
