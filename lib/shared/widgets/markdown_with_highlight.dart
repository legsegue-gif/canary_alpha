...
import 'package:Canary/theme/theme_factory.dart' show getPlatformFontFallback;
...
class _MarkdownTableBlockState extends State<_MarkdownTableBlock> {
  ...
  Future<bool> _savePngBytesToGallery(Uint8List bytes) async {
    final result = await ImageGallerySaverPlus.saveImage(
      bytes,
      quality: 100,
      name: 'canary-table-${DateTime.now().millisecondsSinceEpoch}',
    );
    if (result is Map) {
  ...
  Future<File> _writeTableImageTempFile(Uint8List bytes) async {
    ...
    final file = File(
      p.join(
        dir.path,
        'canary-table-${DateTime.now().millisecondsSinceEpoch}.png',
      ),
    );
  ...
  Future<bool> _writePngToClipboard(Uint8List bytes) async {
    try {
      ...
      if (clipboard != null) {
        final item = DataWriterItem(suggestedName: 'canary-table.png');
