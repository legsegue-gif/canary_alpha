...
class SettingsProvider extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    if (_providerConfigs.isEmpty) {
      ...
      ensureProviderConfig('CanaryIN', defaultName: 'CanaryIN');
  ...
  Future<void> _reloadLocalFontsIfAny() async {
    ...
    if (_appFontLocalPath != null && _appFontLocalPath!.isNotEmpty) {
      final alias = _appFontLocalAlias ?? 'canary_local_app';
    ...
    if (_codeFontLocalPath != null && _codeFontLocalPath!.isNotEmpty) {
      final alias = _codeFontLocalAlias ?? 'canary_local_code';
  ...
  Future<void> _initSearchConnectivityTests() async {
    ...
    for (final s in services) {
      if (s is BingLocalOptions || s is CanaryOptions) {
...
{content}
  ...
  // Search service settings
  Future<bool> unlockCanarySearch() async {
    if (_searchServices.any((s) => s is CanaryOptions)) return false;
    await setSearchServices([
      ..._searchServices,
      CanaryOptions(id: CanaryOptions.builtInId),
    ]);
    return true;
  }
...
class ProviderConfig {
  ...
  static String _apiKeyFromJson(Map<String, dynamic> json) {
    ...
    return id.trim().toLowerCase() == 'canaryin' ? _canaryInPublicApiKey : '';
  }
  ...
  static String _defaultBase(String key) {
    ...
    if (k.contains('canaryin')) return 'https://text.pollinations.ai/openai';
  ...
  static ProviderConfig defaultsFor(String key, {String? displayName}) {
    bool defaultEnabled(String k) {
      ...
      if (s.contains('canaryin')) return true;
    ...
    switch (kind) {
      ...
      case ProviderKind.openai:
        // Special-case CanaryIN default models and overrides
        if (lowerKey.contains('canaryin')) {
          return ProviderConfig(
            id: key,
            enabled: defaultEnabled(key),
            name: displayName ?? key,
            apiKey: _canaryInPublicApiKey,
            baseUrl: _defaultBase(key),
