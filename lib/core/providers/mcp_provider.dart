...
import '../services/mcp/canary_fetch/canary_fetch_server.dart';
...
class McpProvider extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    // Ensure built-in @canary/fetch is present by default
      ...
      if (serversFromMap != null) {
        ...
        serversFromMap.forEach((id, cfgAny) {
          ...
          if (typeLower == 'inmemory') {
            // Built-in @canary/fetch control via isActive; ignore name mismatches silently
            builtinSeen = true;
  ...
  Future<void> connect(String id) async {
    ...
    try {
      ...
      if (server.transport == McpTransportType.inmemory) {
        final engine = CanaryFetchMcpServerEngine();
        final transport = CanaryInMemoryClientTransport(engine);
