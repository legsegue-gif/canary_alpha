...
const String kCanaryVersionFile = '.canary-version';
...
Future<bool> validateInstalledRootfsArchitecture({
  ...
  final marker = File(p.join(rootfsDir.path, kCanaryVersionFile));
...
class EnvironmentInstaller implements EnvironmentManager {
  ...
  Future<void> install({void Function(EnvironmentState)? onProgress}) async {
    ...
    try {
      ...
      _previousState =
          await File(p.join(rootfsDir.path, kCanaryVersionFile)).exists()
          ? env.state
          : null;
  ...
  Future<void> ensureInstalled() async {
    await env.loaded;
    await _resolveEnvDir();
    if (await File(p.join(rootfsDir.path, kCanaryVersionFile)).exists()) {
      if (!await _validateInstalledArchitecture()) return;
  ...
  Future<void> _installBody() async {
    ...
    await _throwIfCancelled();
    final versionPath = p.join(staging.path, kCanaryVersionFile);
  ...
  _readVersionFile() async {
    final file = File(p.join(rootfsDir.path, kCanaryVersionFile));
