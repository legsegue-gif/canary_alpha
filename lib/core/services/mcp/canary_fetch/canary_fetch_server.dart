...
class CanaryFetcher {
  ...
  static Future<http.Response> _fetch(CanaryFetchRequestPayload payload) async {
    try {
  ...
  static String _decodeBody(http.Response resp, {required String contentType}) {
    if (CanaryFetchRequestPayload._charsetOf(contentType) != null) {
      return resp.body;
    }
    try {
  ...
  static String _bounded(String text, CanaryFetchRequestPayload payload) {
    ...
    return '$content\n\n$shown Call canary_fetch with start_index=$end to '
        'continue.]';
  }
...
/// Minimal JSON-RPC server for MCP that serves @canary/fetch tools.
class CanaryFetchMcpServerEngine {
  bool _closed = false;
    ...
    try {
      ...
      switch (method) {
        ...
        case mcp.McpProtocol.methodCallTool:
          ...
          CanaryFetchRequestPayload payload;
          try {
            payload = CanaryFetchRequestPayload.parse(arguments);
          } catch (e) {
...
class CanaryInMemoryClientTransport implements mcp.ClientTransport {
  final CanaryFetchMcpServerEngine _server;
  final _messageController = StreamController<dynamic>.broadcast();
  final _closeCompleter = Completer<void>();
  bool _closed = false;

  CanaryInMemoryClientTransport(this._server);
