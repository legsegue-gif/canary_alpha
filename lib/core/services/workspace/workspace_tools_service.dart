...
class WorkspaceToolsService {
  ...
  static String buildPromptFragment(
    ...
    buf
      ..writeln()
      ..writeln(
        'Cite files as [name](canary://workspace/rel/path), outputs as '
        'canary://chat/outputs/x.txt, images as ![alt](canary://workspace/plot.png). '
  ...
  static String? linkFor(
    ...
    switch (resolved.zone) {
      case WorkspaceZone.workspace:
        ...
        return 'canary://workspace/${CanaryLink.encodePath(rel)}';
      case WorkspaceZone.chat:
        ...
        if (rel == 'attachments' ||
          ...
          return 'canary://chat/${CanaryLink.encodePath(rel)}';
        }
        return 'canary://session/${CanaryLink.encodePath(rel)}';
      case WorkspaceZone.skills:
        ...
        if (rel == null) return null;
        return 'canary://skills/${CanaryLink.encodePath(rel)}';
      case WorkspaceZone.tmp:
        ...
        return rel == null
            ? null
            : 'canary://tmp/${CanaryLink.encodePath(rel)}';
      case WorkspaceZone.external:
        for (final mount in paths.externalMounts) {
          ...
          if (rel != null && mount.externalId != null) {
            return 'canary://mounts/${Uri.encodeComponent(mount.externalId!)}${rel.isEmpty ? '' : '/${CanaryLink.encodePath(rel)}'}';
          }
        }
        return null;
