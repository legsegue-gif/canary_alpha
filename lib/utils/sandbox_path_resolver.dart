...
///
/// Canonical `canary-file:///` URIs are resolved lexically against the cached
...
class SandboxPathResolver {
  ...
  ///
  /// Canonical `canary-file:` URIs are resolved without existence probes.
  static String fix(String path) {
    if (path.isEmpty) return path;

    if (CanaryFileUri.isCanaryFileUri(path)) {
      final docs = _docsDir;
      if (docs == null || docs.isEmpty) return path;
      return CanaryFileUri.resolveToAbsolute(path, root: docs) ?? path;
    }
    ...
    // markers as CanaryFileUri.tryEncodeLegacyAbsolutePath, then generic.
    const subdirs = ['avatars', 'fonts', 'images', 'upload'];
    String? tail; // starts with '/'
    String rootType = 'unknown';

    final encoded = CanaryFileUri.tryEncodeLegacyAbsolutePath(
      raw,
      allowGenericFallback: false,
    );
    if (encoded != null) {
      final segs = CanaryFileUri.decodeToSegments(encoded);
  ...
  /// `canary-file:///` URI when it points under managed app storage.
  ///
  /// Remote (`http`/`https`), `data:`, and already-canonical canary-file URIs
  ...
  /// sandbox markers (`Documents` / `canary` / `app_flutter`·`files`) are
  ...
  static String canonicalize(String uri) {
    ...
    if (CanaryFileUri.isCanaryFileUri(uri)) return uri;
    ...
    final portable = CanaryFileUri.toPortableSlashPath(uri);
    if (portable == null) {
    ...
    if (docs != null && docs.isNotEmpty) {
      ...
      final underRoot = CanaryFileUri.encodeFromAbsolute(portable, root: docs);
      ...
      if (native != portable) {
        final underNative = CanaryFileUri.encodeFromAbsolute(
          native,
          root: docs,
        );
        if (underNative != null) return underNative;
      }
      return CanaryFileUri.tryEncodeLegacyAbsolutePath(
            portable,
            allowGenericFallback: false,
          ) ??
          portable;
  ...
  /// root (because backup files were copied), return the canary-file URI.
  ///
  ...
  static String? tryRemapRestoredManagedAbsolute(String uri) {
    ...
    final abs = CanaryFileUri.resolveToAbsolute(encoded, root: docs);
  ...
  static String? resolveForIo(String path) {
    if (path.isEmpty) return path;
    if (CanaryFileUri.isCanaryFileUri(path)) return fix(path);
  ...
  static String? _remapStructuredIfExists(String abs) {
    final docs = _docsDir;
    if (docs == null || docs.isEmpty) return null;
    final uri = CanaryFileUri.tryEncodeLegacyAbsolutePath(
      abs,
      allowGenericFallback: false,
    );
    if (uri == null) return null;
    final mapped = CanaryFileUri.resolveToAbsolute(uri, root: docs);
