...
class _ImageViewerPageState extends State<ImageViewerPage>
  ...
  Future<void> _saveCurrent() async {
    ...
    try {
      ...
      final name = 'canary-${DateTime.now().millisecondsSinceEpoch}';
      ...
      if (src.startsWith('data:')) {
        ...
        if (i != -1) {
          ...
          temp = await File(
            p.join(
              tmp.path,
              'canary_${DateTime.now().millisecondsSinceEpoch}.png',
            ),
        ...
        if (resp.statusCode >= 200 && resp.statusCode < 300) {
          ...
          temp = await File(
            p.join(
              tmp.path,
              'canary_${DateTime.now().millisecondsSinceEpoch}${ext.isNotEmpty ? ext : '.jpg'}',
            ),
          ).create(recursive: true);
    ...
    if (!ok) {
      try {
        ...
        if (path == null) {
          ...
          path = p.join(
            dir.path,
            'canary_clip_${DateTime.now().millisecondsSinceEpoch}$ext',
          );
  ...
  Future<void> _saveCurrentDesktop() async {
    ...
    try {
      ...
      final defaultName = 'canary-${DateTime.now().millisecondsSinceEpoch}$ext';
